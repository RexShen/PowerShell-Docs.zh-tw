...
您已完成所有建立管理 OData web 服務的必要步驟之後，您必須將它部署為 web 應用程式在 IIS 中。

## <a name="deploying-the-web-service"></a>部署 web 服務

完成下列步驟來部署管理 OData web 服務。

1. 您 web 應用程式在您的 IIS 底下建立目錄`WWWRoot`目錄。

2. 複製 MOF 結構描述檔案，XML 結構描述檔，匯出的 Dll [Microsoft.Management.Odata.Customauthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)並[System.Management.Automation.Remoting.Pssessionconfiguration](/dotnet/api/System.Management.Automation.Remoting.PSSessionConfiguration)介面和應用程式目錄將 web.config 檔案。
...
下列 Windows PowerShell 指令碼會示範如何部署管理 OData web 服務。
