...
ms.locfileid: "62080645"
---
# <a name="implementing-custom-authorization-for-a-management-odata-web-service"></a><span data-ttu-id="533ba-102">實作 Management OData Web 服務的自訂授權</span><span class="sxs-lookup"><span data-stu-id="533ba-102">Implementing Custom Authorization for a Management OData web service</span></span>

<span data-ttu-id="533ba-103">使用 Windows PowerShell Web 服務必須實作的協力廠商[Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)介面，以公開 Windows PowerShell cmdlet。</span><span class="sxs-lookup"><span data-stu-id="533ba-103">Using the Windows PowerShell Web Service requires a third party to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface to expose Windows PowerShell cmdlets.</span></span> <span data-ttu-id="533ba-104">這個介面會執行 web 服務的使用者授權。</span><span class="sxs-lookup"><span data-stu-id="533ba-104">This interface performs user authorization to the web service.</span></span> <span data-ttu-id="533ba-105">之後撰寫的程式碼，以實作介面，您必須將它編譯至 DLL，以用於 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="533ba-105">After writing the code to implement the interface, you must compile it into a DLL to be used in the web application.</span></span>

## <a name="pass-through-authorization"></a><span data-ttu-id="533ba-106">傳遞授權</span><span class="sxs-lookup"><span data-stu-id="533ba-106">Pass-through authorization</span></span>

<span data-ttu-id="533ba-107">最簡單的方式來實作[Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)介面已授權所有使用者的傳遞實作。</span><span class="sxs-lookup"><span data-stu-id="533ba-107">The simplest way to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface is a pass-through implementation that authorizes all users.</span></span> <span data-ttu-id="533ba-108">此範例會提供任何安全性和 s 僅提供作為示範了如何實作介面。</span><span class="sxs-lookup"><span data-stu-id="533ba-108">This example provides no security, and s provided only as an illustration of how to implement the interface.</span></span> <span data-ttu-id="533ba-109">實作[Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)介面必須覆寫兩個方法：[Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser)並[Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId)。</span><span class="sxs-lookup"><span data-stu-id="533ba-109">An implementation of the  [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface must override two methods: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span></span> <span data-ttu-id="533ba-110">在此範例中， [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser)一律會傳回**System.Security.Principal.WindowsIdentity**與目前的使用者相關聯的物件。</span><span class="sxs-lookup"><span data-stu-id="533ba-110">In this example, the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) always returns the **System.Security.Principal.WindowsIdentity** object associated with the current user.</span></span>
...
### <a name="role-based-authorization"></a><span data-ttu-id="533ba-111">以角色為基礎的授權</span><span class="sxs-lookup"><span data-stu-id="533ba-111">Role-based authorization</span></span>

<span data-ttu-id="533ba-112">下列範例會實作以角色為基礎的授權原則。</span><span class="sxs-lookup"><span data-stu-id="533ba-112">The following example implements a role-based authorization policy.</span></span> <span data-ttu-id="533ba-113">原則會定義於 XML 檔案位於主要的應用程式目錄的 web.config 和 MOF 和 XML 的對應結構描述檔案。</span><span class="sxs-lookup"><span data-stu-id="533ba-113">The policy is defined in an XML file that resides in the main application directory with the web.config and MOF and XML mapping schema files.</span></span> <span data-ttu-id="533ba-114">如需有關如何設定授權結構描述檔案的資訊，請參閱[設定的角色為基礎的授權](./configuring-role-based-authorization.md)。</span><span class="sxs-lookup"><span data-stu-id="533ba-114">For information about how to configure the authorization schema file, see [Configuring Role-based Authorization](./configuring-role-based-authorization.md).</span></span> <span data-ttu-id="533ba-115">此範例的第一個部分會實作[Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser)並[Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId)方法。</span><span class="sxs-lookup"><span data-stu-id="533ba-115">The first part of the sample implements the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) methods.</span></span> <span data-ttu-id="533ba-116">在此情況下，介面方法呼叫中的方法`RbacSystem`類別 （定義如下） 來執行實際的工作，檢查使用者的權限。</span><span class="sxs-lookup"><span data-stu-id="533ba-116">In this case, the interface methods call methods in the `RbacSystem` class (defined below) that do the actual work of checking the permissions for the user.</span></span>
...
<span data-ttu-id="533ba-117">下列範例會建立會剖析 XML 檔案中的授權原則的類別。</span><span class="sxs-lookup"><span data-stu-id="533ba-117">The following example creates a class that parses authorization policy in the XML file.</span></span>
...
```
 ...
 <span data-ttu-id="533ba-118">下列類別分別代表群組和使用者進行。</span><span class="sxs-lookup"><span data-stu-id="533ba-118">The following classes represent groups and users, respectively.</span></span>
