...
您會定義管理 OData web 服務公開 （expose） 藉由使用公用資源結構描述的 MOF 檔案的資源。 每個資源會定義為類別，以在檔案中，而且屬性會定義為類別成員。 如需有關在 MOF 檔案中使用的結構描述的詳細資訊，請參閱[公開的資源結構描述](./public-resource-schema.md)。

## <a name="example-mof-file"></a>範例 MOF 檔案

下列檔案會定義服務和處理資源。 每個資源對應至物件，可將一組 Windows PowerShell cmdlet。 屬性會對應至這些 cmdlet 使用的參數。

每兩個資源都包含屬於複雜類型的屬性。 複雜型別定義為類別已修改與`ComplexType`限定詞。
