...
以下是 StopProc01 cmdlet 範例的程式碼範例。 這是`Stop-Process`cmdlet 的範例中所述[建立 Cmdlet 會修改系統](../cmdlet/creating-a-cmdlet-that-modifies-the-system.md)。 `Stop-Process`指令程式設計來停止處理程序會擷取使用 Get-proc cmdlet (中所述[建立您的第一個 Cmdlet](../cmdlet/creating-a-cmdlet-without-parameters.md))。
