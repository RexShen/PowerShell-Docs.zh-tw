...
使用會話狀態時，windows powershell 執行時間會針對特定`Get-Location`的 Windows powershell 提供者（例如、 `Set-Location`、 `Pop-Location`和`Push-Location` Cmdlet）提供數個位置 Cmdlet。 您可以使用`Get-Help` Cmdlet 來取得這些 location Cmdlet 的相關資訊。
...
這個類別是衍生自[Cmdletprovider](/dotnet/api/System.Management.Automation.Provider.CmdletProvider)基類的。 下表列出這個類別所公開的 Cmdlet。 除了列出的`Get-PSDrive` Cmdlet 之外，指令程式（由會話狀態公開）是用來取出可用磁片磁碟機的相關 Cmdlet。
...
|Cmdlet|定義|
|------------|----------------|
|`Clear-Item`|清除指定位置上專案的目前內容，並以提供者所指定的「清除」值來取代它。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
|`Get-Item`|從指定的位置抓取專案，並串流結果物件。|
|`Invoke-Item`|在指定路徑上叫用專案的預設動作。|
|`Set-Item`|在指定的位置設定具有指定值的專案。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
|`Resolve-Path`|解析 Windows PowerShell 路徑的萬用字元，並串流路徑資訊。|
|`Test-Path`|測試指定的路徑， `true`如果`false`存在則傳回，否則傳回。 這個指令程式會實作為支援`IsContainer` [Cmdletprovider. Writeitemobject *](/dotnet/api/System.Management.Automation.Provider.CmdletProvider.WriteItemObject)方法的參數。|
...
|Cmdlet|定義|
|------------|----------------|
|`Copy-Item`|將專案從一個位置複製到另一個位置。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
|`Get-Childitem`|抓取位於指定位置的子專案，並將它們串流為物件。|
|`New-Item`|在指定的位置建立新專案，並串流結果物件。|
|`Remove-Item`|從指定的位置移除專案。|
|`Rename-Item`|重新命名指定位置的專案。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
...
|`Move-Item`|將專案移到指定的位置。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
...
除了衍生自其中一個 Windows PowerShell 基類以外，您的 Windows PowerShell 提供者也可以衍生自下列一個或多個提供者介面，以支援其他功能。 這個區段會定義這些介面和各項支援的 Cmdlet。 它不會描述介面支援的 Cmdlet 的參數。 Cmdlet 參數資訊可在`Get-Command`線上使用和`Get-Help` Cmdlet 取得。
...
|Cmdlet|定義|
|------------|----------------|
|`Add-Content`|將指示的值長度附加至指定專案的內容。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
|`Clear-Content`|將指定專案的內容設定為 "clear" 值。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
|`Get-Content`|抓取指定專案的內容，並串流結果物件。|
|`Set-Content`|取代所指定專案的現有內容。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
...
> [!NOTE]
> 這些`Path` Cmdlet 上的參數表示專案的路徑，而不是識別屬性。

|Cmdlet|定義|
|------------|----------------|
|`Clear-ItemProperty`|將指定專案的屬性設定為 "clear" 值。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
|`Get-ItemProperty`|從指定的專案抓取屬性，並串流結果物件。|
|`Set-ItemProperty`|設定具有指定之值的指定專案屬性。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
...
|Cmdlet|定義|
|------------|----------------|
|`Copy-ItemProperty`|將屬性從指定的專案複製到另一個專案。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
|`Move-ItemProperty`|將屬性從指定的專案移至另一個專案。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
|`New-ItemProperty`|在指定的專案上建立屬性，並串流結果物件。|
|`Remove-ItemProperty`|移除指定專案的屬性。|
|`Rename-ItemProperty`|重新命名指定專案的屬性。 除非已指定其`PassThru`參數，否則這個 Cmdlet 不會透過管線傳遞輸出物件。|
