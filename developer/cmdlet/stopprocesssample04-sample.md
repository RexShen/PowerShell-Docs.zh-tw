...
此範例示範如何撰寫 cmdlet 會宣告參數集，請指定預設參數設定，以及可接受輸入的物件。 此 cmdlet 會類似於`Stop-Process`Windows PowerShell 2.0 所提供的 cmdlet。

### <a name="how-to-build-the-sample-by-using-visual-studio"></a>如何使用 Visual Studio 建置範例。

1. 安裝 Windows PowerShell 2.0 sdk，瀏覽至 StopProcessSample04 資料夾。 預設位置是 C:\Program Files (x86) \Microsoft SDKs\Windows\v7.0\Samples\sysmgmt\WindowsPowerShell\csharp\StopProcessSample04。
...
下列程式碼顯示宣告參數集，並指定預設參數設定，以及可接受的輸入的物件的停止程序 cmdlet 實作。

這個範例會顯示輸入的物件、 如何宣告參數集，以及如何指定設定為使用預設參數。
