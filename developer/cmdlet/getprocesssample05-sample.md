...
此範例示範 Get-proc cmdlet 的完整版本。

## <a name="how-to-build-the-sample-using-visual-studio"></a>如何使用 Visual Studio 建置範例。

1. 開啟 Windows 檔案總管並巡覽至範例目錄下的 GetProcessSample05 目錄。

   安裝 Windows PowerShell 2.0 sdk，瀏覽至 GetProcessSample05 資料夾。 預設位置是 C:\Program Files (x86) \Microsoft SDKs\Windows\v7.0\Samples\sysmgmt\WindowsPowerShell\csharp\GetProcessSample05。

2. 按兩下方案 (.sln) 檔案的圖示。 這是在 Visual Studio 中開啟範例專案。

3. 在 **建置**功能表上，選取**建置方案**。

   如需範例程式庫將會建置在預設的 \bin 或 \bin\debug 目錄中。
...
- 指定參數可以接受來自管線的輸入。 輸入可以取自物件或物件的屬性名稱為參數名稱相同的屬性的值。
