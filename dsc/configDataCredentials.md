...
本例使用 `PSDesiredStateConfiguration` 內建 DSC 資源模組的 [Group](https://msdn.microsoft.com/powershell/dsc/groupresource) 資源。
它會建立本機群組並新增或移除成員。
...
第一個錯誤訊息有文件的 URL。
這個連結說明如何使用 [ConfigurationData](https://msdn.microsoft.com/powershell/dsc/configdata) 結構和憑證加密密碼。
