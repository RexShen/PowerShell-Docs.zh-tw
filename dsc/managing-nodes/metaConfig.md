...
| RebootNodeIfNeeded| bool| 將此設為`$true`以允許重新啟動節點使用的資源`$global:DSCMachineStatus`旗標。 否則，您將必須手動重新啟動任何設定所需的節點。 預設值為 `$false`。 若要在重新啟動條件是由 DSC 以外的項目 (例如 Windows Installer) 所制定的情況下使用此設定，請將此設定與 [xPendingReboot](https://github.com/powershell/xpendingreboot) \(英文\) 模組結合。|
...
> [!NOTE]
> LCM 會啟動**ConfigurationModeFrequencyMins**循環為基礎：
>
> - 使用套用新的 metaconfig `Set-DscLocalConfigurationManager`
> - 電腦重新開機
>
> 任何條件，其中的計時器程序發生時將會在 30 秒內偵測到的損毀，而週期將會重新啟動。
> 並行作業可能會延遲週期從開始，如果這項作業的持續時間超過設定的循環頻率下, 一個計時器將不會啟動。
>
> 範例中，metaconfig 在 15 分鐘提取頻率設定，並提取，就會發生在 T1。  節點不會完成 16 分鐘的工作。  會忽略第一個 15 分鐘的週期，以及下一個提取將會發生在 T1 + 15 + 15。
