...
5. 這會在名為 `SampleIISInstall` 的新資料夾中建立 `localhost.mof` 檔案。
   使用 [Move-Item](/powershell/module/microsoft.powershell.management/move-item) Cmdlet 將該檔案重新命名為 `Pending.mof`，再將其移至 VHD 上正確的位置。 例如：
...
您也可以將中繼設定用為 VHD 的 `MetaConfig.mof` 檔案插入 VHD，將電腦設定成在初始開機時提取組態以提取在初始啟動的組態 (請參閱 [設定本機設定管理員 (LCM)](../managing-nodes/metaConfig.md))。
...
2. [設定 DSC Web 提取伺服器](../pull-server/pullServer.md)，並將 **SampleIISInistall** 組態儲存到適當的資料夾。
