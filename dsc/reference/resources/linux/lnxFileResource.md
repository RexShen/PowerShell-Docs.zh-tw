...
步驟 1：從遠端來源 (http、https 或 ftp) 複製檔案：在 Linux 上以想要的內容建立檔案，並暫存於您要設定的節點所能存取的網頁或 FTP 伺服器上。 以該檔案的 Web 或 FTP URL 定義 __nxFile__ 資源的 __SourcePath__ 屬性。
...
步驟 2：在設定 __$OFS__ 屬性後，透過 [Get-Content](https://technet.microsoft.com/library/hh849787.aspx) 讀取 PowerShell 指令碼中的檔案內容以使用 Linux 分行字元。
