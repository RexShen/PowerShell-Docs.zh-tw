...
在選擇 DNS 記錄名稱時，請牢記解決方案架構。 如果使用負載平衡，則用來保護 HTTPS 流量的憑證就需要與 DNS 記錄共用相同的名稱。 同樣地，如果使用高可用性的檔案共用，則會使用叢集的虛擬名稱。

案例 |最佳做法
:---|:---
測試環境 |可能的話，請重新產生規劃的生產環境。 伺服器主機名稱適用於簡單的設定。 若無 DNS 可用，主機名稱可使用 IP 位址。|
單一節點部署 |建立指向伺服器主機名稱的 DNS CNAME 記錄。|
高可用性部署 |如果用戶端會透過負載平衡解決方案連接，請建立參考主機名稱的虛擬 IP 和 CNAME 記錄的主機名稱。 如果 DNS 循環配置資源要用於將用戶端要求分散到各提取伺服器，您必須設定名稱記錄包含所有已部署提取伺服器執行個體的主機名稱。|
...
#### <a name="designing-for-high-availability"></a>設計高可用性  
高可用性架構中可以部署提取伺服器角色。 Web 服務角色可以是負載平衡的，而包含 DSC 模組和 DSC 設定的檔案和資料夾可以位於高可用性存放裝置中。

請記住，一旦設定和模組傳遞至目標節點，執行測試及設定組態所需的全部資料都會儲存在本機的每個節點。 只有變更是從提取伺服器傳遞。 除非部署作用中，否則提取伺服器的服務中斷不會造成中斷。  通常只有最大的環境才能保證高可用性。

設定高可用性的提取伺服器環境，需要決定如何將用戶端要求分散到多個伺服器節點，以及如何在這些節點之間共用所需的伺服器檔案。
...
在多部伺服器設定為提取伺服器，且彼此間的連線負載平衡的高可用性案例中，從這些伺服器取得的資源和設定要完全相同，是很重要的事。 完成這項作業的最佳方法為將此內容儲存在高可用性的位置，例如叢集的檔案共用。 您可以在每部伺服器的設定中指定共用的位置。如需共用存放裝置選項的詳細資訊，請參閱＜用於應用程式資料的向外延展檔案伺服器概觀＞。
...
您會如何將 GUID 對應至伺服器或角色？這份資料儲存在何處？|
您要使用什麼樣的程序來設定用戶端電腦？它會如何與您的建立及儲存設定 GUID 程序整合？|
