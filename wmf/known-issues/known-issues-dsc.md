...
**解決方法：** 以 [資料編密] 或 [金鑰編密] 的金鑰使用方式，與 [文件加密增強] 金鑰使用方式 (1.3.6.1.4.1.311.80.1) 重新建立憑證。 如需詳細資訊，請參閱 [Protect-CmsMessage](/powershell/module/Microsoft.PowerShell.Security/Protect-CmsMessage)。
...
**解決方法：** 在提高權限的 PowerShell 工作階段 (以系統管理員身分執行) 中執行下列命令來刪除 DSCEngineCache.mof︰
...
**解決方法：** 在提高權限的 PowerShell 工作階段 (以系統管理員身分執行) 中執行下列命令︰
...
**解決方法：** 偵錯 `Get-DscConfiguration` Cmdlet 時，請勿按下 CTRL+C。
...
**解決方法：** 完成對 `Get-DscConfiguration` 所啟動的作業進行偵錯，如[偵錯 DSC 資源](/powershell/dsc/troubleshooting/debugResource)中所述。

## <a name="no-verbose-error-messages-are-shown-in-debugmode"></a>在 DebugMode 中不顯示詳細的錯誤訊息

如果 LCM 處於 **DebugMode**，則 DSC 資源不會顯示詳細的錯誤訊息。

**解決方法：** 停用 **DebugMode** 以查看資源中的詳細訊息
...
當 LCM 在偵錯模式中執行時，`Invoke-DscResource` Cmdlet 不會提供要連線以進行偵錯的 Runspace 相關資訊。 如需詳細資訊，請參閱[偵錯 DSC 資源](/powershell/dsc/troubleshooting/debugResource)。

**解決方法：** 使用 `Get-PSHostProcessInfo`、`Enter-PSHostProcess`、`Get-Runspace` 及 `Debug-Runspace` Cmdlet 來探索並附加至 Runspace，以對 DSC 資源進行偵錯。
...
**解決方法：** 在不同的部分設定中使用不同資源名稱 (即使是相同的資源)。
...
**解決方法：** 在 DSC 資源類別本身中定義所有變數和函式。 沒有 `$script` 範圍變數/函式。
...
**解決方法：** 使用 Credential 屬性 (如果有的話)。 範例 ServiceSet 和 WindowsFeatureSet
...
**解決方法：** 無。 不過，使用 IntelliSense 時，在 ISE 中撰寫設定會反映有關 **PsDscRunAsCredential** 屬性的正確中繼資料。
...
**解決方法：** 藉由將 **ModuleSpecification** 物件定義為 **ModuleName** 參數，並指定 **RequiredVersion** 索引鍵來匯入所需的版本，如下所示：
