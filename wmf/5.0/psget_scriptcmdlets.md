...
---
# <a name="powershellget-cmdlets-for-script-management"></a><span data-ttu-id="b9f37-102">適用於指令碼管理的 PowerShellGet Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-102">PowerShellGet Cmdlets for Script Management</span></span>

## <a name="find-script-cmdlet"></a><span data-ttu-id="b9f37-103">Find-Script Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-103">Find-Script cmdlet</span></span>
<span data-ttu-id="b9f37-104">Find-Script Cmdlet 可讓您使用不同的搜尋準則探索指令碼檔案，例如名稱、標記、篩選、命令名稱、版本範圍、正確版本、所有版本，包括其相依性和特定或所有已註冊的存放庫。</span><span class="sxs-lookup"><span data-stu-id="b9f37-104">Find-Script cmdlet lets you to discover the script files with different search criteria like name, tag, filter, command name, version range, exact version, all versions, including its dependencies and from specific or all registered repositories.</span></span>

<span data-ttu-id="b9f37-105">使用範例：</span><span class="sxs-lookup"><span data-stu-id="b9f37-105">Example usage:</span></span>
```powershell
...
## <a name="save-script-cmdlet"></a><span data-ttu-id="b9f37-106">Save-Script Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-106">Save-Script cmdlet</span></span>
<span data-ttu-id="b9f37-107">Save-Script Cmdlet 可讓您將指令碼檔案儲存到指定的位置，藉以檢閱指令碼檔案。</span><span class="sxs-lookup"><span data-stu-id="b9f37-107">Save-Script cmdlet lets you to review the script file by saving it to a specified location.</span></span>
```powershell
...
## <a name="install-script-and-get-installedscript-cmdlets"></a><span data-ttu-id="b9f37-108">Install-Script 和 Get-InstalledScript Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-108">Install-Script and Get-InstalledScript cmdlets</span></span>
<span data-ttu-id="b9f37-109">Install-Script Cmdlet 可讓您將特定的指令碼檔案及其相依性安裝到指定的範圍。</span><span class="sxs-lookup"><span data-stu-id="b9f37-109">Install-Script cmdlet lets you to install a specific script file along with its dependencies to the specified scope.</span></span> <span data-ttu-id="b9f37-110">根據預設，指令碼會安裝在 AllUsers 範圍。</span><span class="sxs-lookup"><span data-stu-id="b9f37-110">By default, scripts are installed to the AllUsers scope.</span></span> <span data-ttu-id="b9f37-111">Get-InstalledScript Cmdlet 可讓您取得使用 Install-Script Cmdlet 安裝的指令碼檔案清單。</span><span class="sxs-lookup"><span data-stu-id="b9f37-111">Get-InstalledScript cmdlet lets you to get the list of script files which were installed using Install-Script cmdlet.</span></span>

<span data-ttu-id="b9f37-112">使用注意事項：為允許在安裝指令碼之後管理和尋找指令碼，Install-script 將會在 $home\Documents\WindowsPowerShell\Scripts 建立用於儲存指令碼的預設資料夾，並將此資料夾加入到您的 PATH 環境。</span><span class="sxs-lookup"><span data-stu-id="b9f37-112">Use note: To allow management and locating of scripts once they are installed, Install-script will create a default folder for storing scripts at $home\Documents\WindowsPowerShell\Scripts, and add that folder to your PATH environment.</span></span> <span data-ttu-id="b9f37-113">如果修改路徑是個問題，請使用 Save-Script 不要使用 Install-Script。</span><span class="sxs-lookup"><span data-stu-id="b9f37-113">If modifying the path is a concern, use Save-Script instead of Install-Script.</span></span> <span data-ttu-id="b9f37-114">Get-InstalledScripts 和 Uninstall-Script 只能搭配使用 Install-Script 放在系統上的指令碼。</span><span class="sxs-lookup"><span data-stu-id="b9f37-114">Get-InstalledScripts and Uninstall-Script can only work with scripts placed on the system using Install-Script.</span></span>
```powershell
...
<span data-ttu-id="b9f37-115">您也可以使用 Get-Command –Name &lt;InstalledScriptFileName&gt; 取得它。</span><span class="sxs-lookup"><span data-stu-id="b9f37-115">You can also use Get-Command –Name &lt;InstalledScriptFileName&gt; to get it.</span></span> <span data-ttu-id="b9f37-116">第一次使用指定範圍時，PATH 環境變數中會加入兩個安裝位置。</span><span class="sxs-lookup"><span data-stu-id="b9f37-116">Two install locations are added to the PATH environment variable on first use of a specified scope.</span></span>
```powershell
...
## <a name="update-script-cmdlet"></a><span data-ttu-id="b9f37-117">Update-Script Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-117">Update-Script cmdlet</span></span>
<span data-ttu-id="b9f37-118">Update-Script Cmdlet 可讓您使用 Install-Script Cmdlet 就地更新已安裝的指令碼檔案。</span><span class="sxs-lookup"><span data-stu-id="b9f37-118">Update-Script cmdlet lets you to do in-place update of the script files which were installed using Install-Script cmdlet.</span></span>
```powershell
...
## <a name="uninstall-script-cmdlet"></a><span data-ttu-id="b9f37-119">Uninstall-Script Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-119">Uninstall-Script cmdlet</span></span>
```powershell
...
## <a name="new-scriptfileinfo-and-test-scriptfileinfo-cmdlets"></a><span data-ttu-id="b9f37-120">New-ScriptFileInfo 和 Test-ScriptFileInfo Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-120">New-ScriptFileInfo and Test-ScriptFileInfo cmdlets</span></span>
<span data-ttu-id="b9f37-121">New-ScriptFileInfo Cmdlet 可讓您建立具中繼資料的新指令碼檔案，例如版本、GUID、作者和描述等等。Test-ScriptFileInfo Cmdlet 可讓您驗證並取得指令碼檔案中繼資料。</span><span class="sxs-lookup"><span data-stu-id="b9f37-121">New-ScriptFileInfo cmdlet lets you to create a new script file with metadata like Version, Guid, Author, and Description, etc. Test-ScriptFileInfo cmdlet lets you to validate and get the script file metadata.</span></span>
```powershell
...
## <a name="update-scriptfileinfo-cmdlet"></a><span data-ttu-id="b9f37-122">Update-ScriptFileInfo Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-122">Update-ScriptFileInfo cmdlet</span></span>
<span data-ttu-id="b9f37-123">Update-ScriptFileInfo Cmdlet 可讓您更新現有的指令碼檔案中繼資料。</span><span class="sxs-lookup"><span data-stu-id="b9f37-123">Update-ScriptFileInfo cmdlet lets you to update the existing script file metadata.</span></span>
```powershell
...
## <a name="register-psrepository-and-set-psrepository-cmdlets-with-script-sharing-support"></a><span data-ttu-id="b9f37-124">具指令碼共用支援的 Register-PSRepository 和 Set-PSRepository Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-124">Register-PSRepository and Set-PSRepository cmdlets with script sharing support</span></span>
<span data-ttu-id="b9f37-125">使用 Register-PSRepository/Set-PSRepository Cmdlet 將 **ScriptSourceLocation** 和 **ScriptPublishLocation** 加入 PSRepository 中。</span><span class="sxs-lookup"><span data-stu-id="b9f37-125">Use Register-PSRepository/Set-PSRepository cmdlets to add the **ScriptSourceLocation** and **ScriptPublishLocation** to the PSRepository.</span></span>
```powershell
...
## <a name="publish-script-cmdlet"></a><span data-ttu-id="b9f37-126">Publish-Script Cmdlet</span><span class="sxs-lookup"><span data-stu-id="b9f37-126">Publish-Script cmdlet</span></span>
<span data-ttu-id="b9f37-127">Publish-Script Cmdlet 可讓您發佈具有效中繼資料的指令碼檔案，例如版本、GUID、作者和描述等等。</span><span class="sxs-lookup"><span data-stu-id="b9f37-127">Publish-Script cmdlet lets you to publish your script file with valid metadata like Version, Guid, Author, and Description, etc.</span></span>
```powershell
