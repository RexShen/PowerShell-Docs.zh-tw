...
`Find-PackageProvider` Cmdlet 會在向 PowerShellGet 註冊的套件來源中尋找相符的 PackageManagement 提供者。 這些是可使用 `Install-PackageProvider` Cmdlet 進行安裝的套件提供者。 根據預設，這包括 PowerShell Gallery 提供之具有 'PackageManagement' 和 'Provider' 標記的模組。
