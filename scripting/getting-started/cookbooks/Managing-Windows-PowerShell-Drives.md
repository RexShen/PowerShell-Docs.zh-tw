...
您可以使用參照所有 Windows 磁碟機的方式來參照新的 Windows PowerShell 磁碟機 \-\-，亦即輸入磁碟機名稱並加上冒號 (**:**)。

Windows PowerShell 磁碟機可以讓許多工作變得更簡單。 例如，Windows 登錄中的某些重要機碼具有極長的路徑，使得它們不容易存取且難以記住。 重要設定資訊位於 **HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion**。 若要檢視及變更 CurrentVersion 登錄機碼中的項目，您可以建立根目錄為該機碼的 Windows PowerShell 磁碟機，方式是輸入下列命令：
...
您可以使用 **Remove\-PSDrive** Cmdlet 從 Windows PowerShell 刪除磁碟機。 **Remove\-PSDrive** Cmdlet 使用方式很簡單，您只需要提供 Windows PowerShell 磁碟機名稱，即可刪除特定的 Windows PowerShell 磁碟機。
