...
**Format** Cmdlet 為 **Format\-Wide**、**Format\-List**、**Format\-Table** 和 **Format\-Custom**。 本使用者手冊只會說明 **Format\-Wide**、**Format\-List** 和 **Format\-Table** Cmdlet。

每個 Format Cmdlet 都有預設屬性，可在未指定要顯示的特定屬性時使用。 每個 Cmdlet 也都會使用相同的參數名稱 (**Property**) 來指定您想要顯示的屬性。 因為 **Format\-Wide** 只會顯示單一屬性，所以其 **Property** 參數只使用單一值，但 **Format\-List** 和 **Format\-Table** 的屬性參數會接受屬性名稱清單。
...
**Format\-List** Cmdlet 以清單形式顯示物件，並在個別行上標記和顯示每個屬性：
...
雖然 **Format\-List** 命令適用於顯示詳細資訊，但是，如果您想要輸出包括許多項目的概觀，則較簡單的表格式檢視通常更為有用。
