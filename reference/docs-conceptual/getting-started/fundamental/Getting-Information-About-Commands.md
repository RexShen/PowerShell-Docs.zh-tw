...
Windows PowerShell `Get-Command` Cmdlet 能取得您目前工作階段中所有可用的命令。 當您在 Windows PowerShell 命令提示字元鍵入 `Get-Command` 時，會看到如下輸出：
...
> [!NOTE]
> 星號 (\*) 在 Windows PowerShell 命令引數中用於萬用字元比對。 \* 表示「符合一或多個任意字元」。 您可以鍵入 `Get-Command a*` 來尋找開頭為字母 "a" 的所有命令。 與 Cmd.exe 中萬用字元比對不同的是，Windows PowerShell 的萬用字元也會比對句號。
