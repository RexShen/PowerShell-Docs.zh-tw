...
1. 從資料夾中刪除 NuGet 特定項目。
1. 重新命名資料夾。 預設資料夾名稱通常是 `<name>.<version>`。 如果模組標記為發行前版本，則版本可以包含 `-prerelease`。 請將資料夾重新命名為只有模組名稱。 例如，`azurerm.storage.5.0.4-preview` 會成為 `azurerm.storage`。
1. 將資料夾複製到 `$env:PSModulePath value` 的其中一個資料夾。 `$env:PSModulePath` 是以分號分隔的路徑集合，PowerShell 會在此集合中尋找模組。
...
1. 解壓縮 NuGet 套件內容。
1. 資料夾中的 `.PS1` 檔案可直接從此位置使用。
1. 您可以刪除此資料夾中的 NuGet 特定項目。
