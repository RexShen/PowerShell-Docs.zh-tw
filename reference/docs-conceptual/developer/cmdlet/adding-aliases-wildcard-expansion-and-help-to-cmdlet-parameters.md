...
您的 Cmdlet 必須定義支援系統修改和使用者意見反應的參數。 Cmdlet 應定義 `Name` 參數或對等，讓 Cmdlet 能夠以某種識別碼來修改系統。 此外，此 Cmdlet 應該會定義 `Force` 和 @no__t 1 參數。 如需這些參數的詳細資訊，請參閱[建立可修改系統的 Cmdlet](./creating-a-cmdlet-that-modifies-the-system.md)。
...
除了使用[Aliasattribute](/dotnet/api/System.Management.Automation.AliasAttribute)屬性以外，Windows PowerShell 執行時間也會執行部分名稱比對，即使未指定別名也一樣。 例如，如果您的 Cmdlet 具有 `FileName` 參數，而且這是以 `F` 開頭的唯一參數，則使用者可以輸入 `Filename`、`Filenam`、`File`、`Fi` 或 `F`，而且仍然會將專案辨識為 `FileName` 參數。
...
若要允許選取多個物件，您的 Cmdlet 可以使用[Wildcardpattern](/dotnet/api/System.Management.Automation.WildcardPattern)和[Wildcardoptions](/dotnet/api/System.Management.Automation.WildcardOptions)類別，為參數輸入提供萬用字元擴充支援。 萬用字元模式的範例包括 lsa *、@no__t 4.9.0-和 [a-c] \*。 當模式包含應逐字使用的字元時，請使用後引號字元（'）做為換用字元。
...
下列程式碼示範如何設定萬用字元選項，並定義用來解析此 Cmdlet 的 `Name` 參數的萬用字元模式。
...
- 現在，請進行下列專案，以停止符合萬用字元模式 "* note @ no__t-0" 的所有處理常式。 在您停止每個符合模式的處理常式之前，系統會先提示您。
