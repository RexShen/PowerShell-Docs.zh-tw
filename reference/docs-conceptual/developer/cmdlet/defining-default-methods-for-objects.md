...
> [!NOTE]
> 下列各節中的範例來自 Windows PowerShell 安裝目錄中的 `Types.ps1xml` 類型檔案（`$PSHOME`）。 如需詳細資訊，請參閱[關於類型. types.ps1xml](/powershell/module/microsoft.powershell.core/about/about_types.ps1xml)。
