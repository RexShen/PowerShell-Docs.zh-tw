...
指定觸發條件的腳本區塊。 當此腳本評估為時 `true` ，就會符合條件，並且會使用資料表專案。
...
選取條件必須指定至少一個腳本區塊或屬性名稱，但不能同時指定兩者。 如需如何使用選取條件的詳細資訊，請參閱 [定義使用視圖專案或專案的條件](./defining-conditions-for-displaying-data.md)。
