...
設定專案（格式） ViewDefinitions 元素（格式） View 元素（format） Controls 專案（格式）控制項的控制項專案（格式） CustomControl 元素，用於控制項的 View （Format） CustomEntries 元素CustomControl for View （format） CustomEntry 元素 for CustomEntries for view （format） CustomItem 元素 for view （格式） CustomEntry for view （格式）之控制項的 CustomItem
...
下列各節說明屬性、子專案，以及 `NewLine` 元素的父元素。
