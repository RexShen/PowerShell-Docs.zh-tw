---
title: 執行 Management OData web 服務的自訂授權 |Microsoft Docs
ms.custom: ''
ms.date: 09/13/2016
ms.reviewer: ''
ms.suite: ''
...
---
# <a name="implementing-custom-authorization-for-a-management-odata-web-service"></a><span data-ttu-id="cc24f-102">實作 Management OData Web 服務的自訂授權</span><span class="sxs-lookup"><span data-stu-id="cc24f-102">Implementing Custom Authorization for a Management OData web service</span></span>

<span data-ttu-id="cc24f-103">使用 Windows PowerShell Web 服務需要協力廠商來執行[CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)介面，以公開 Windows powershell Cmdlet。</span><span class="sxs-lookup"><span data-stu-id="cc24f-103">Using the Windows PowerShell Web Service requires a third party to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface to expose Windows PowerShell cmdlets.</span></span> <span data-ttu-id="cc24f-104">此介面會對 web 服務執行使用者授權。</span><span class="sxs-lookup"><span data-stu-id="cc24f-104">This interface performs user authorization to the web service.</span></span> <span data-ttu-id="cc24f-105">撰寫程式碼以執行介面之後，您必須將它編譯成要在 web 應用程式中使用的 DLL。</span><span class="sxs-lookup"><span data-stu-id="cc24f-105">After writing the code to implement the interface, you must compile it into a DLL to be used in the web application.</span></span>

## <a name="pass-through-authorization"></a><span data-ttu-id="cc24f-106">傳遞授權</span><span class="sxs-lookup"><span data-stu-id="cc24f-106">Pass-through authorization</span></span>

<span data-ttu-id="cc24f-107">執行[CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)介面最簡單的方式，是授權所有使用者的傳遞執行。</span><span class="sxs-lookup"><span data-stu-id="cc24f-107">The simplest way to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface is a pass-through implementation that authorizes all users.</span></span> <span data-ttu-id="cc24f-108">這個範例不提供安全性，而且只提供如何執行介面的說明。</span><span class="sxs-lookup"><span data-stu-id="cc24f-108">This example provides no security, and s provided only as an illustration of how to implement the interface.</span></span> <span data-ttu-id="cc24f-109">[CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)介面的執行必須覆寫兩個方法： [CustomAuthorization. AuthorizeUser 和](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser)。 [CustomAuthorization. GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId). 和的程式。</span><span class="sxs-lookup"><span data-stu-id="cc24f-109">An implementation of the  [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface must override two methods: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span></span> <span data-ttu-id="cc24f-110">在此範例中， [AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser)一律會傳回與目前使用者相關聯的**WindowsIdentity**物件。（CustomAuthorization）</span><span class="sxs-lookup"><span data-stu-id="cc24f-110">In this example, the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) always returns the **System.Security.Principal.WindowsIdentity** object associated with the current user.</span></span>
...
### <a name="role-based-authorization"></a><span data-ttu-id="cc24f-111">以角色為基礎的授權</span><span class="sxs-lookup"><span data-stu-id="cc24f-111">Role-based authorization</span></span>

<span data-ttu-id="cc24f-112">下列範例會實行以角色為基礎的授權原則。</span><span class="sxs-lookup"><span data-stu-id="cc24f-112">The following example implements a role-based authorization policy.</span></span> <span data-ttu-id="cc24f-113">原則會定義于位於主要應用程式目錄中的 XML 檔案中，並具有 web.config 和 MOF 和 XML 對應架構檔案。</span><span class="sxs-lookup"><span data-stu-id="cc24f-113">The policy is defined in an XML file that resides in the main application directory with the web.config and MOF and XML mapping schema files.</span></span> <span data-ttu-id="cc24f-114">如需有關如何設定授權架構檔案的詳細資訊，請參閱設定以[角色為基礎的授權](./configuring-role-based-authorization.md)。</span><span class="sxs-lookup"><span data-stu-id="cc24f-114">For information about how to configure the authorization schema file, see [Configuring Role-based Authorization](./configuring-role-based-authorization.md).</span></span> <span data-ttu-id="cc24f-115">此範例的第一個部分會執行[CustomAuthorization. AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser)和[CustomAuthorization. GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId)方法。</span><span class="sxs-lookup"><span data-stu-id="cc24f-115">The first part of the sample implements the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) methods.</span></span> <span data-ttu-id="cc24f-116">在此情況下，介面方法會呼叫 `RbacSystem` 類別中的方法（定義如下），以執行檢查使用者許可權的實際工作。</span><span class="sxs-lookup"><span data-stu-id="cc24f-116">In this case, the interface methods call methods in the `RbacSystem` class (defined below) that do the actual work of checking the permissions for the user.</span></span>
...
<span data-ttu-id="cc24f-117">下列範例會建立類別，以剖析 XML 檔案中的授權原則。</span><span class="sxs-lookup"><span data-stu-id="cc24f-117">The following example creates a class that parses authorization policy in the XML file.</span></span>
...
```
 ...
 <span data-ttu-id="cc24f-118">下列類別分別代表群組和使用者。</span><span class="sxs-lookup"><span data-stu-id="cc24f-118">The following classes represent groups and users, respectively.</span></span>
