...
使用 Windows PowerShell Web 服務需要協力廠商來執行[CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization)介面，以公開 Windows powershell Cmdlet。 此介面會對 web 服務執行使用者授權。 撰寫程式碼以執行介面之後，您必須將它編譯成要在 web 應用程式中使用的 DLL。
