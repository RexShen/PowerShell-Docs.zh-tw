...
以下是 GetProc03 範例 Cmdlet 的程式碼範例。 這是[新增處理管線輸入的參數](../cmdlet/adding-parameters-that-process-pipeline-input.md)中所述的 `Get-Process` Cmdlet 範例。 這個 `Get-Process` Cmdlet 使用可接受管線物件輸入的 `Name` 參數，根據提供的名稱從本機電腦上抓取進程資訊，然後在命令列顯示進程的相關資訊。
