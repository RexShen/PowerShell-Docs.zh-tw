...
PowerShell Core 也支援在所有平台上 (Windows、macOS 及 Linux) 透過 SSH 進行 PowerShell 遠端處理 (PSRP)。 雖然目前在生產環境中不支援此功能，但您可以從[這裡](/powershell/scripting/learn/remoting/SSH-Remoting-in-PowerShell-Core)深入瞭解如何設定此功能。
