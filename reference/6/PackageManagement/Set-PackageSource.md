...
會 `Set-PackageSource` 取代指定封裝提供者的封裝來源。 套件來源一律受套件提供者管理。
...
指出 `Set-PackageSource` 強制 **PackageManagement** 自動安裝封裝提供者。
...
指定代表要變更之封裝的封裝來源識別碼物件。 套件來源識別碼是 Cmdlet 結果的一部分 `Get-PackageSource` 。
...
指定目前的套件來源位置。 此值可以是 URI、檔案路徑或封裝提供者所支援的任何其他目的地格式。
...
指定套件來源的新位置。 此值可以是 URI、檔案路徑或封裝提供者所支援的任何其他目的地格式。
...
指出來源是信任的封裝提供者。 信任來源不會提示您進行驗證，以安裝套件。
