...
Escape (`` `e ``) 字元最常用來指定虛擬終端序列 (ANSI escape 序列) ，以修改文字色彩和其他文字屬性，例如粗體和底線。 這些序列也可用於資料指標的定位和滾動。 PowerShell 主機必須支援虛擬終端機序列。 您可以檢查的布林值 `$Host.UI.SupportsVirtualTerminal` ，以判斷是否支援這些 ANSI 序列。

如需 ANSI 轉義順序的詳細資訊，請參閱 [ANSI_escape_code](https://en.wikipedia.org/wiki/ANSI_escape_code)。
...
## <a name="unicode-character-ux"></a>Unicode 字元 (' u {x} ) 

Unicode escape sequence (`` `u{x} ``) 可讓您依其程式碼點的十六進位標記法來指定任何 Unicode 字元。 這包括基本多語系平面上面的 Unicode 字元 ( # A0 `0xFFFF`) ，其中包含像是 **大拇指** () 字元等表情字元 `` `u{1F44D} `` 。 Unicode escape 序列至少需要一個十六進位數位，且最多可支援六個十六進位數位。 順序的最大十六進位值為 `10FFFF` 。
