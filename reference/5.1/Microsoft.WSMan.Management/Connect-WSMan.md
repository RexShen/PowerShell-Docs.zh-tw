...
此參數是設計用於下列狀況：如果有多部電腦會建立遠端連線至同一部執行 Windows PowerShell 的電腦。
...
* 您不需要建立 WS-Management 工作階段，就可以在遠端電腦上執行管理命令或查詢遠端電腦上的管理資料。 您可以使用 Invoke-WSManAction 的 *ComputerName* 參數和 set-wsmaninstance 來完成這項作業。 當您使用 *ComputerName* 參數時，Windows PowerShell 會建立用於單一命令的暫時連接。 該命令執行之後，此連線即會關閉。
