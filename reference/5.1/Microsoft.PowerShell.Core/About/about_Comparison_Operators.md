...
`-replace`運算子會使用正則運算式來取代具有指定值的所有或部分值。 您可以使用 `-replace` 運算子進行許多系統管理工作，例如重新命名檔案。 例如，下列命令會將所有 .txt 檔案的副檔名變更為 .log：

```powershell
Get-ChildItem *.txt | Rename-Item -NewName { $_.name -replace '\.txt$','.log' }
```

運算子的語法 `-replace` 如下所示，其中 `<original>` 預留位置代表要取代的字元，而 `<substitute>` 預留位置代表將取代這些字元的字元：
...
您也可以使用正則運算式，以動態方式使用捕捉群組和替代來取代文字。 如需詳細資訊，請參閱 [about_Regular_Expressions](about_Regular_Expressions.md)。

#### <a name="substitutions-in-regular-expressions"></a>在規則運算式中執行替代

此外，也可以在字串中參考捕捉群組 \<substitute\> 。
這是藉由使用 `$` 群組識別碼之前的字元來完成。

參考「捕獲群組」的兩個方式是依 **編號** 和 **名稱**
...
> [!WARNING]
> 由於 `$` 字元是在字串展開中使用，因此您必須使用常值字串搭配替代，或將字元換用 `$` 。
>
> ```powershell
