...
_進程_ 範圍包含目前進程或 PowerShell 會話中可用的環境變數。 這份變數清單繼承自父進程，而且是由 _電腦_ 和 _使用者_ 範圍中的變數所構成。
...
- PSModuleAnalysisCachePath
  ...
  快取的預設檔案名為 `ModuleAnalysisCache` 。 若要變更快取的預設位置，請在啟動 PowerShell 之前設定環境變數。 對此環境變數所做的變更只會影響子進程。
