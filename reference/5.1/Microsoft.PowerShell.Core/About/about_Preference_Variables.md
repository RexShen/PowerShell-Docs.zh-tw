...
此範例顯示 `$DebugPreference` 變數預設值 **SilentlyContinue** 的效果。 依預設， `Write-Debug` 不會顯示 Cmdlet 的偵錯工具訊息，而且會繼續處理。 使用 **Debug** 參數時，它會覆寫單一命令的喜好設定。 系統會提示使用者進行確認。
...
- **NormalView** ： (預設) 為大部分的使用者設計的詳細觀點。 包含錯誤的描述，以及包含在錯誤中的物件名稱。
...
這個範例示範當的值 `$ErrorView` 是預設的 **NormalView** 時，如何顯示錯誤。 `Get-ChildItem` 用來尋找不存在的檔案。
...
- **繼續** ：顯示資訊訊息，並繼續執行。
- **暫** 止：執行命令後自動暫停工作流程工作 `Write-Information` ，以允許使用者在繼續之前查看訊息。 工作流程可依使用者的意願繼續。
...
判斷在 PowerShell 會話中允許的別名數目。 預設值為 **4096** ，而且應該足以滿足大部分的用途。 您可以調整 `$MaximumAliasCount` 以符合您的需求。

**有效的值** ： 1024-32768 (`Int32`) 
...
判斷指定的會話中允許的 PowerShell 磁片磁碟機數目。 例如，PowerShell 提供者所公開的檔案系統磁片磁碟機和資料存放區，會顯示為磁片磁碟機，例如 `Alias:` 和 `HKLM:` 磁片磁碟機。
...
**有效的值** ： 256-32768 (`Int32`) 

**預設值** ：256

代表每個保留錯誤的物件會儲存在 `$Error` 自動變數中。 `$Error` 包含錯誤記錄物件的陣列。 最新的錯誤是陣列中的第一個物件 `$Error[0]` 。

若要計算您系統上的錯誤，請使用 `$Error` 陣列的 **count** 屬性。

```powershell
$Error.count
```

若要顯示特定的錯誤，請使用 `[0]` 陣列標記法來查看最近的錯誤。
...
**Force** 參數會覆寫 **ErrorRecord** 物件的特殊格式，並還原為傳統格式。 若要顯示 **ErrorRecord** 物件的屬性，請輸入下列命令：

```powershell
$Error[0] | Format-List -Property * -Force
```

在此範例中，會 `$Error.Count` 顯示錯誤的數目。 若要刪除錯誤記錄中的所有錯誤，請使用 `Clear` 錯誤陣列的方法。
...
若要尋找錯誤陣列的所有屬性和方法，請使用 `Get-Member` Cmdlet 與其 **InputObject** 參數。 當您使用 **InputObject** 參數時，會 `Get-Member` 顯示集合的屬性和方法。

```powershell
Get-Member -InputObject $Error
```

當您使用管線將物件集合傳送至時 `Get-Member` ，會 `Get-Member` 顯示集合中物件的屬性和方法。
...
若要查看會話中的函數，請使用 `Function:` powershell 提供者所公開的 powershell 磁片磁碟機 `Function` 。 如需提供者的詳細資訊 `Function` ，請 [about_Function_Provider](about_Function_Provider.md)。
...
判斷指定的會話中允許的變數數目，包括自動變數、喜好設定變數，以及您在命令和腳本中建立的變數。

**有效的值** ： 1024-32768 (`Int32`) 

**預設值** ：4096

若要查看會話中的變數，請使用 `Get-Variable` powershell `Variable:` 磁片磁碟機和 powershell 提供者的 Cmdlet 和功能 `Variable` 。 如需詳細資訊，請參閱 [about_Variable_Provider](about_Variable_Provider.md)。
...
**預設值** ： ASCIIEncoding 物件 (ASCIIEncoding) 
