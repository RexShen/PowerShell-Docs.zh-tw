...
使用具有 **ValueFromRemainingArguments** 之集合的已知問題，會將傳入的集合視為單一元素。

下列範例示範此已知問題。 **其餘** 參數 **應包含****索引 0** 和 **索引 1** 的 **兩個** 參數。
相反地，這兩個元素會合並為單一實體。
...
**ValidateRange** 屬性會指定每個參數或變數值的數值範圍。 如果任何值超出該範圍，PowerShell 就會產生錯誤。 在下列範例中， **嘗試** 參數的值必須介於0到10之間。
