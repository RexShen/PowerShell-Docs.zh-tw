...
包含目前正在作業系統中使用的文化特性名稱。 文化特性會決定專案（例如數位、貨幣及日期）的顯示格式，並且儲存在 **system.string 物件中。** 用 `Get-Culture` 來顯示電腦的文化特性。 `$PSCulture` 包含 **Name** 屬性的值。
...
| **CLRVersion**            | Common language runtime 的版本    |
|                           |  (CLR)                                          |
| **PSCompatibleVersions**  | 相容的 PowerShell 版本    |
|                           | 使用目前的版本                      |
| **PSEdition**             | 這個屬性的值為 ' Desktop '，適用于 |
|                           | Windows Server 和 Windows 用戶端版本。   |
|                           | 這個屬性的值為 < Core     |
|                           | 在 Nano Server 或上執行的 PowerShell       |
|                           | Windows IOT。                                  |
