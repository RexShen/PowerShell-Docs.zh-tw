...
使用 Enter-PSSession Cmdlet 來啟動遠端電腦的互動式會話。 您可以使用 Enter-PSSession 的 ComputerName 參數來建立互動式會話的暫時連接。 或者，您可以使用 Session 參數，在 Windows PowerShell 會話中執行互動式會話 (PSSession) 。
...
您可以使用包含作業名詞 (工作 Cmdlet) 的 Cmdlet 來管理任何 Cmdlet 所建立的任何作業。 許多具有 AsJob 參數的 Cmdlet 都不會使用 Windows PowerShell 遠端功能，因此您甚至可以在未設定為進行遠端處理的電腦上使用它們，也不符合遠端處理的需求。
