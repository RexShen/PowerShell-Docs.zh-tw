...
**連接 PSSession** Cmdlet 會重新連接到已中斷連線的使用者管理 Windows PowerShell 會話 ( **pssession** ) 。
...
根據預設，Windows PowerShell 不會重新導向連線，但是您可以使用此參數允許它重新導向連線。
...
若要使用預設連接埠於 Windows PowerShell 遠端功能，請為 HTTP 指定連接埠 5985，或是為 HTTPS 指定連接埠 5986。
...
若要設定接聽程式，請在 Windows PowerShell 提示字元中輸入下列兩個命令：
