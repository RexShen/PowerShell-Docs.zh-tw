...
在腳本區塊中，第一個命令會使用 Cmdlet 來抓取所有正在執行的服務 `Where-Object` 。 這些服務會透過管道傳送至 `ForEach-Object` Cmdlet。 此 `ForEach-Object` Cmdlet 會建立新的 [`[System.Management.Automation.CompletionResult]`](/dotnet/api/system.management.automation.completionresult) 物件，並以管線變數) 所表示的目前服務 (值填入該物件 `$_` 。
