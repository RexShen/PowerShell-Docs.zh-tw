...
`$command`變數 `Get-EventLog` 會儲存格式化為腳本區塊的命令。 會 `Invoke-Command` 執行儲存在 `$command` S1 和 S2 遠端電腦上的命令。
...
**ComputerName** 參數會指定以逗號分隔的電腦名稱稱清單。 電腦的清單包含 localhost 值（代表本機電腦）。 **ConfigurationName** 參數會指定替代的會話設定。 **ScriptBlock** 參數會執行 `Get-EventLog` 以取得每部電腦的 Windows PowerShell 事件記錄檔。
...
`$Log`變數會儲存事件記錄檔的名稱，Windows PowerShell。 `Invoke-Command`Cmdlet 會 `Get-EventLog` 在 Server01 上執行，以從事件記錄檔取得十個最新的事件。 **LogName** 參數的值為 `$Log` 變數，此變數的前面會加上 `Using` 範圍修飾符，以指出它是在本機會話中建立，而不是在遠端會話中建立的。
