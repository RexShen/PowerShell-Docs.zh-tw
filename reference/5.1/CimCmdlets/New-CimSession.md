...
使用其他 Cmdlet （例如 Cmdlet）時，您可以使用此名稱來參考 CIM 會話 [`Get-CimSession`](Get-CimSession.md) 。
...
指定遠端電腦上要供此連線使用的網路連接埠。
若要連線到遠端電腦，遠端電腦必須接聽連線使用的連接埠。
預設連接埠是 5985 (用於 HTTP 的 WinRM 連接埠) 和 5986 (用於 HTTPS 的 WinRM 連接埠)。
