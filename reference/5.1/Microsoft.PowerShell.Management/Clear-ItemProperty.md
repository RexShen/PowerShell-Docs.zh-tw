...
當您使用 *PassThru* 參數時，會 `Clear-ItemProperty` 產生代表已清除專案屬性的 **PSCustomObject** 物件。
否則，此 Cmdlet 不會產生任何輸出。

## 注意

您可以使用 `Clear-ItemProperty` 刪除登錄值中的資料而不刪除值。 如果值的資料類型是二進位或 DWORD，清除資料會將值設定為零。 否則，值會是空的。

此 `Clear-ItemProperty` Cmdlet 是針對與任何提供者公開的資料搭配使用所設計。 若要列出工作階段中可用的提供者，請輸入 `Get-PSProvider`。 如需詳細資訊，請參閱 about_Providers。
