...
此命令會將偵錯工具附加到電腦上的 Windows PowerShell 處理程序。
...
此命令會將偵錯工具附加到電腦上的 Windows PowerShell 處理程序。
它會使用 **Get-Process** Cmdlet 取得電腦上的 Windows PowerShell 處理程序，然後使用管線運算子 (|) 將處理程序傳送至 **Debug-Process** Cmdlet。
...
此命令會將偵錯工具附加到電腦上目前的 Windows PowerShell 處理程序。

此命令會使用 $PID 自動變數，其中包含目前 Windows PowerShell 處理程序的處理程序識別碼。
然後，它會使用管線運算子 (|) 將處理序識別碼傳送至 **Debug 進程** Cmdlet。

如需 $PID 自動變數的詳細資訊，請參閱 about_Automatic_Variables。

### 範例 7︰將偵錯工具附加到多部電腦上指定的處理程序

```
PS C:\> Get-Process -ComputerName "Server01", "Server02" -Name "MyApp" | Debug-Process
```

此命令將偵錯工具附加到 Server01 和 Server02 電腦上的 MyApp 處理程序。

此命令會使用 **Get-Process** Cmdlet 來取得 Server01 和 Server02 電腦上的 MyApp 處理程序。
它會使用管線運算子將處理程序傳送至 Debug-Process Cmdlet，這會附加偵錯工具。

### 範例 8︰將偵錯工具附加到使用 InputObject 參數的處理程序
...
此命令會將偵錯工具附加到本機電腦上的 Windows PowerShell 處理程序。

第一個命令會使用 **Get-Process** Cmdlet 來取得電腦上的 Windows PowerShell 處理程序。
