...
從 Windows PowerShell 5.0 開始，使用類似其他物件導向程式設計語言的正式語法和語義，新增了語言來定義類別和其他使用者定義類型。 目標是要讓開發人員和 IT 專業人員能夠將 Windows PowerShell 用於更廣泛的使用案例、簡化 PowerShell 成品的開發（例如 DSC 資源），以及加快管理面的涵蓋範圍。
...
[建置自訂的 Windows PowerShell 預期狀態設定資源](/powershell/scripting/dsc/resources/authoringResource)
