...
此 `Get-Credential` Cmdlet 會提示使用者輸入密碼或使用者名稱和密碼。 您可以使用 **Message** 參數，在命令列提示字元中指定自訂的訊息。
...
當您輸入命令時，系統會提示您輸入使用者名稱和密碼。 當您輸入要求的資訊時，Cmdlet 會建立代表使用者認證的 **PSCredential** 物件，並將其儲存在 `$c` 變數中。
...
**PromptForCredential** 方法是使用 Cmdlet 的替代方法 `Get-Credential` 。 當您使用 **PromptForCredential** 時，可以指定出現在提示字元中的標題、訊息和使用者名稱。
