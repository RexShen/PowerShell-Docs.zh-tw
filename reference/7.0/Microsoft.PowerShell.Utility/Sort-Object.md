...
在 PowerShell 6 中， `Sort-Object` [ **下** 一個] 參數是的替代方法 `Select-Object` 。 例如： `Get-Process | Sort-Object -Property WS -Bottom 5` 。
...
這個範例會使用兩個屬性來排序物件、 **狀態** 和 **DisplayName** 。 **狀態** 會以遞減順序排序，而 **DisplayName** 會以遞增順序排序。
...
當您使用 **Top、Top** 或 **穩定** 參數時，排序的物件會依照 **Bottom** `Sort-Object` 排序準則相等時所收到的順序來傳遞。 在此範例中，我們會將數位1到20的值 ' 模數 3 ' 排序。 模數值的範圍是從零到二。
...
第一個排序的輸出會依模數值正確分組，但不會在模數範圍內排序個別專案。 第二個排序使用 **穩定** 選項來傳回穩定的排序。

## PARAMETERS

### -下

指定要從已排序物件陣列的結尾取得的物件數目。 這會導致穩定的排序。
...
指定要從已排序物件陣列的開頭取得的物件數目。 這會導致穩定的排序。
...
排序的物件會依照排序準則相等時所收到的順序來傳遞。

此參數是在 PowerShell v 6.2.0 中新增的。
