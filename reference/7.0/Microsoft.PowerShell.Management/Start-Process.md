...
指出此 Cmdlet 會載入儲存在目前使用者的登錄機碼中的 Windows 使用者設定檔 `HKEY_USERS` 。 參數不適用於非 Windows 系統。
...
在目前的主控台視窗中啟動新的處理程序。 根據預設，在 Windows 上，PowerShell 會開啟新的視窗。 在非 Windows 系統上，您永遠不會收到新的終端機視窗。
...
指定新進程的開始位置。 預設值是要啟動的可執行檔或檔的位置。 提供的路徑會被視為常值路徑。 不支援萬用字元。 您必須將路徑括在單引號中 (`'`) 如果路徑名稱包含的字元會被視為萬用字元。
...
- 在 Windows 上，最常見的使用案例 `Start-Process` 是使用 **Wait** 參數來封鎖進度，直到新的進程結束為止。 在非 Windows 系統上，很少需要這樣做，因為命令列應用程式的預設行為相當於 `Start-Process -Wait` 。

- `Start-Process`在非 Windows 系統上使用時，您永遠不會收到新的終端機視窗。
