...
PowerShell 7 引進了 **WorkingDirectory** 參數，可指定背景工作的初始工作目錄。 如果未指定參數，則 `Start-Job` 預設為啟動作業之呼叫端的目前工作目錄。
...
`Start-Job` 使用 **Name** 參數來指定易記的作業名稱 **GetMappingFiles** 。 **InitializationScript** 參數會執行匯入 **MapFunctions** 模組的腳本區塊。 **ScriptBlock** 參數會執行 `Get-Map` ，並將 `Set-Content` 資料儲存在 **Path** 參數所指定的位置。
...
**WorkingDirectory** 可讓您指定工作的替代目錄，讓您可以執行腳本或開啟檔案。 在此範例中，背景工作會指定與目前目錄位置不同的工作目錄。
...
這個範例的目前工作目錄是 `C:\Test` 。 `Start-Job` 使用 **WorkingDirectory** 參數來指定作業的工作目錄。 **ScriptBlock** 參數會使用 `$PWD` 來顯示作業的工作目錄。 `Receive-Job` 顯示背景作業的輸出。
**AutoRemoveJob** 會刪除工作，並 **等候** 抑制命令提示字元，直到收到所有結果為止。

### 範例10：使用 ArgumentList 參數來指定陣列
...
### 範例11：在 Windows PowerShell 5.1 中執行作業

此範例會使用值為 **5.1** 的 **PSVersion** 參數，在 Windows PowerShell 5.1 會話中執行作業。
...
指定要用於執行工作的 PowerShell 版本。
當 **PSVersion** 的值為 **5.1** 時，會在 Windows PowerShell 5.1 會話中執行作業。
若為任何其他值，則會使用目前版本的 PowerShell 來執行此作業。

此參數是在 PowerShell 7 中新增的，且僅適用于 Windows。
...
從 PowerShell 7 開始， **RunAs32** 參數無法在64位 PowerShell () 上運作 `pwsh` 。
如果在64位 PowerShell 中指定 **RunAs32** ，則會擲回 `Start-Job` 終止例外狀況錯誤。
若要使用 RunAs32 啟動32位 PowerShell (`pwsh`) 處理 **RunAs32** 程式，您必須安裝32位 powershell。

在32位 PowerShell 中， **RunAs32** 會強制工作在32位的進程中執行，即使是在64位的作業系統上也一樣。
...
指定背景工作的初始工作目錄。 如果未指定參數，則會從預設位置執行此作業。 預設位置是啟動工作之呼叫端的目前工作目錄。
