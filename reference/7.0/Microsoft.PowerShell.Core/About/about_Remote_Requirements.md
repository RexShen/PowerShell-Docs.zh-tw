...
若要尋找具有 ComputerName 參數但未使用 PowerShell 遠端的 Cmdlet，請閱讀 Cmdlet 的 ComputerName 參數描述。
...
## <a name="basic-authentication-limitations-on-linux-and-macos"></a>Linux 和 macOS 上的基本驗證限制

從 Linux 或 macOS 系統連接到 Windows 時，不支援透過 HTTP 進行基本驗證。 您可以在目標伺服器上安裝憑證，透過 HTTPS 使用基本驗證。 憑證的 CN 名稱必須符合主機名稱、未過期或已撤銷。 自我簽署憑證可用於測試用途。

如需其他詳細資料，請參閱 [如何：設定 WINRM FOR HTTPS](https://support.microsoft.com/help/2019527/how-to-configure-winrm-for-https) 。

下列命令會從提高許可權的命令提示字元中執行，並使用已安裝的憑證，在 Windows 上設定 HTTPS 接聽程式。
...
在 Linux 或 macOS 端上，選取 [基本] 以進行驗證和-UseSSl。

> 注意：基本驗證無法與網域帳戶搭配使用。需要本機帳戶，而且該帳戶必須在 Administrators 群組中。
...
透過 HTTPS 進行基本驗證的替代方案是 Negotiate。 這會導致用戶端和伺服器之間的 NTLM 驗證，以及透過 HTTP 加密的承載。

以下說明如何搭配使用 Negotiate 與新的-PSSession：
...
> [!NOTE]
> Windows Server 需要額外的登錄設定，才能讓系統管理員（而非內建的系統管理員）使用 NTLM 進行連接。
> [針對遠端](/windows/win32/winrm/authentication-for-remote-connections)連線，請參閱 [驗證中的協商驗證] 底下的 LocalAccountTokenFilterPolicy 登錄設定
