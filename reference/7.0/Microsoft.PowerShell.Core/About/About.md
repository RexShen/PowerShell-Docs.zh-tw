...
描述語句如何 `Continue` 立即將程式流程傳回至程式迴圈的頂端。
...
PowerShell 中的檔案會在 `Format.ps1xml` powershell 主控台中定義物件的預設顯示。
...
說明如何為 Cmdlet 和 advanced 函式的參數設定自訂的預設值。
...
提供 PowerShell 會話的詳細資訊，以及它們在遠端命令中扮演的角色。

### [about_PSSessions](about_PSSessions.md)
說明 (Pssession) 的 PowerShell 會話，並說明如何建立與遠端電腦的持續連線。

### [about_pwsh](about_pwsh.md)
說明如何使用 pwsh 命令列工具。 顯示語法並描述命令列參數。 pwsh 會啟動 PowerShell 會話。
...
描述您可以用來控制 PowerShell 如何解讀命令或參數中下一個字元的特殊字元。
...
描述可搭配 Microsoft .NET Framework 類型使用的運算子。

### [about_Types.ps1xml](about_Types.ps1xml.md)
說明如何使用 Types.ps1xml 檔案來擴充 PowerShell 中使用的物件類型。
