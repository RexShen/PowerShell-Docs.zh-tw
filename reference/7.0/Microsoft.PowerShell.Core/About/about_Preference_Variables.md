...
|             <span data-ttu-id="0809c-113">變數</span><span class="sxs-lookup"><span data-stu-id="0809c-113">Variable</span></span>             |       <span data-ttu-id="0809c-114">預設值</span><span class="sxs-lookup"><span data-stu-id="0809c-114">Default Value</span></span>       |
| -------------------------------- | ------------------------- |
...
#### <a name="detailed-explanation"></a><span data-ttu-id="0809c-173">詳細說明</span><span class="sxs-lookup"><span data-stu-id="0809c-173">Detailed explanation</span></span>
...
<span data-ttu-id="0809c-218">此範例使用 **Debug** 參數搭配的值 `$false` 來抑制單一命令的訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-218">This example uses the **Debug** parameter with a value of `$false` to suppress the message for a single command.</span></span> <span data-ttu-id="0809c-219">不會顯示 debug 訊息，也不會停止處理。</span><span class="sxs-lookup"><span data-stu-id="0809c-219">The debug message isn't displayed and processing isn't stopped.</span></span>
...
- <span data-ttu-id="0809c-229">**中斷** -進入發生錯誤時或引發例外狀況時的偵錯工具。</span><span class="sxs-lookup"><span data-stu-id="0809c-229">**Break** - Enter the debugger when an error occurs or when an exception is raised.</span></span>
...
<span data-ttu-id="0809c-249">此範例會顯示 `$ErrorActionPreference` 預設值 [ **繼續** ]。</span><span class="sxs-lookup"><span data-stu-id="0809c-249">This example shows the `$ErrorActionPreference` default value, **Continue** .</span></span> <span data-ttu-id="0809c-250">產生非終止錯誤。</span><span class="sxs-lookup"><span data-stu-id="0809c-250">A non-terminating error is generated.</span></span> <span data-ttu-id="0809c-251">訊息隨即顯示，並且繼續處理。</span><span class="sxs-lookup"><span data-stu-id="0809c-251">The message is displayed and processing continues.</span></span>
...
- <span data-ttu-id="0809c-261">**ConciseView** ： (預設) 提供簡潔的錯誤訊息，以及 advanced 模組產生器的重構視圖。</span><span class="sxs-lookup"><span data-stu-id="0809c-261">**ConciseView** : (Default) Provides a concise error message and a refactored view for advanced module builders.</span></span> <span data-ttu-id="0809c-262">如果錯誤來自命令列，則為單一行錯誤訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-262">If the error is from command line it's a single line error message.</span></span> <span data-ttu-id="0809c-263">否則，您會收到包含錯誤的多行錯誤訊息，以及顯示該錯誤在該行中發生之錯誤的指標。</span><span class="sxs-lookup"><span data-stu-id="0809c-263">Otherwise, you receive a multiline error message that contains the error and a pointer to the error showing where it occurs in that line.</span></span> <span data-ttu-id="0809c-264">如果終端機支援虛擬終端機，則會使用 ANSI 色彩代碼來提供色彩強調。</span><span class="sxs-lookup"><span data-stu-id="0809c-264">If the terminal supports Virtual Terminal, then ANSI color codes are used to provide color accent.</span></span> <span data-ttu-id="0809c-265">您可以在中變更輔色 `$Host.PrivateData.ErrorAccentColor` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-265">The Accent color can be changed at `$Host.PrivateData.ErrorAccentColor`.</span></span> <span data-ttu-id="0809c-266">使用指令程式可完整 `Get-Error` 查看完整的錯誤，包括內部例外狀況。</span><span class="sxs-lookup"><span data-stu-id="0809c-266">Use `Get-Error` cmdlet for a comprehensive detailed view of the fully qualified error, including inner exceptions.</span></span>
  ...
  <span data-ttu-id="0809c-267">**ConciseView** 已在 PowerShell 7 中新增。</span><span class="sxs-lookup"><span data-stu-id="0809c-267">**ConciseView** was added in PowerShell 7.</span></span>
...
<span data-ttu-id="0809c-321">**Log \* 事件** 喜好設定變數如下所示：</span><span class="sxs-lookup"><span data-stu-id="0809c-321">The **Log\*Event** preference variables are as follows:</span></span>
...
### <a name="maximumhistorycount"></a><span data-ttu-id="0809c-340">\$MaximumHistoryCount</span><span class="sxs-lookup"><span data-stu-id="0809c-340">\$MaximumHistoryCount</span></span>
...
#### <a name="examples"></a><span data-ttu-id="0809c-354">範例</span><span class="sxs-lookup"><span data-stu-id="0809c-354">Examples</span></span>
...
### <a name="psdefaultparametervalues"></a><span data-ttu-id="0809c-390">\$PSDefaultParameterValues</span><span class="sxs-lookup"><span data-stu-id="0809c-390">\$PSDefaultParameterValues</span></span>
...
<span data-ttu-id="0809c-408">如需自動匯入模組的詳細資訊，請參閱 [about_Modules](about_Modules.md)。</span><span class="sxs-lookup"><span data-stu-id="0809c-408">For more information about automatic importing of modules, see [about_Modules](about_Modules.md).</span></span>
...
<span data-ttu-id="0809c-419">`$PSSessionApplicationName`喜好設定變數是在本機電腦上設定的，但它會指定遠端電腦上的接聽程式。</span><span class="sxs-lookup"><span data-stu-id="0809c-419">The `$PSSessionApplicationName` preference variable is set on the local computer, but it specifies a listener on the remote computer.</span></span> <span data-ttu-id="0809c-420">如果您指定的應用程式名稱不存在於遠端電腦上，用來建立會話的命令會失敗。</span><span class="sxs-lookup"><span data-stu-id="0809c-420">If the application name that you specify doesn't exist on the remote computer, the command to establish the session fails.</span></span>

### <a name="pssessionconfigurationname"></a><span data-ttu-id="0809c-421">\$PSSessionConfigurationName</span><span class="sxs-lookup"><span data-stu-id="0809c-421">\$PSSessionConfigurationName</span></span>

<span data-ttu-id="0809c-422">指定用於在目前會話中建立之 **pssession** 的預設會話設定。</span><span class="sxs-lookup"><span data-stu-id="0809c-422">Specifies the default session configuration that is used for **PSSessions** created in the current session.</span></span>

<span data-ttu-id="0809c-423">此喜好設定變數是在本機電腦上設定的，但它會指定位於遠端電腦上的會話設定。</span><span class="sxs-lookup"><span data-stu-id="0809c-423">This preference variable is set on the local computer, but it specifies a session configuration that's located on the remote computer.</span></span>

<span data-ttu-id="0809c-424">變數的值 `$PSSessionConfigurationName` 是完整的資源 URI。</span><span class="sxs-lookup"><span data-stu-id="0809c-424">The value of the `$PSSessionConfigurationName` variable is a fully qualified resource URI.</span></span>

<span data-ttu-id="0809c-425">預設值 `http://schemas.microsoft.com/PowerShell/microsoft.PowerShell` 表示遠端電腦上的 **Microsoft PowerShell** 會話設定。</span><span class="sxs-lookup"><span data-stu-id="0809c-425">The default value `http://schemas.microsoft.com/PowerShell/microsoft.PowerShell` indicates the **Microsoft.PowerShell** session configuration on the remote computer.</span></span>

<span data-ttu-id="0809c-426">如果您只指定設定名稱，則會在前面加上下列架構 URI：</span><span class="sxs-lookup"><span data-stu-id="0809c-426">If you specify only a configuration name, the following schema URI is prepended:</span></span>

`http://schemas.microsoft.com/PowerShell/`

<span data-ttu-id="0809c-427">您可以使用 **ConfigurationName** `New-PSSession` 、 `Enter-PSSession` 或 Cmdlet 的 ConfigurationName 參數來覆寫預設值，並為特定會話選取不同的會話設定 `Invoke-Command` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-427">You can override the default and select a different session configuration for a particular session by using the **ConfigurationName** parameter of the `New-PSSession`, `Enter-PSSession`, or `Invoke-Command` cmdlets.</span></span>

<span data-ttu-id="0809c-428">您可以隨時變更此變數的值。</span><span class="sxs-lookup"><span data-stu-id="0809c-428">You can change the value of this variable at any time.</span></span> <span data-ttu-id="0809c-429">當您這樣做時，請記住，您選取的會話設定必須存在於遠端電腦上。</span><span class="sxs-lookup"><span data-stu-id="0809c-429">When you do, remember that the session configuration that you select must exist on the remote computer.</span></span> <span data-ttu-id="0809c-430">如果不是，用來建立使用會話設定之會話的命令會失敗。</span><span class="sxs-lookup"><span data-stu-id="0809c-430">If it doesn't, the command to create a session that uses the session configuration fails.</span></span>

<span data-ttu-id="0809c-431">當遠端使用者建立連接到這部電腦的會話時，這個喜好設定變數不會決定要使用的本機會話設定。</span><span class="sxs-lookup"><span data-stu-id="0809c-431">This preference variable doesn't determine which local session configurations are used when remote users create a session that connects to this computer.</span></span>
<span data-ttu-id="0809c-432">不過，您可以使用本機會話設定的許可權來判斷哪些使用者可能會使用這些設定。</span><span class="sxs-lookup"><span data-stu-id="0809c-432">However, you can use the permissions for the local session configurations to determine which users may use them.</span></span>

### <a name="pssessionoption"></a><span data-ttu-id="0809c-433">\$PSSessionOption</span><span class="sxs-lookup"><span data-stu-id="0809c-433">\$PSSessionOption</span></span>

<span data-ttu-id="0809c-434">在遠端會話中建立 advanced user 選項的預設值。</span><span class="sxs-lookup"><span data-stu-id="0809c-434">Establishes the default values for advanced user options in a remote session.</span></span>
<span data-ttu-id="0809c-435">這些選項喜好設定會覆寫會話選項的系統預設值。</span><span class="sxs-lookup"><span data-stu-id="0809c-435">These option preferences override the system default values for session options.</span></span>

<span data-ttu-id="0809c-436">`$PSSessionOption`變數包含 **PSSessionOption** 物件。</span><span class="sxs-lookup"><span data-stu-id="0809c-436">The `$PSSessionOption` variable contains a **PSSessionOption** object.</span></span> <span data-ttu-id="0809c-437">如需詳細資訊，請參閱 [PSSessionOption](/dotnet/api/system.management.automation.remoting.pssessionoption)。</span><span class="sxs-lookup"><span data-stu-id="0809c-437">For more information, see [System.Management.Automation.Remoting.PSSessionOption](/dotnet/api/system.management.automation.remoting.pssessionoption).</span></span>
<span data-ttu-id="0809c-438">物件的每個屬性都代表一個會話選項。</span><span class="sxs-lookup"><span data-stu-id="0809c-438">Each property of the object represents a session option.</span></span> <span data-ttu-id="0809c-439">例如， **NoCompression** 屬性會在會話期間開啟資料壓縮。</span><span class="sxs-lookup"><span data-stu-id="0809c-439">For example, the **NoCompression** property turns of data compression during the session.</span></span>

<span data-ttu-id="0809c-440">根據預設， `$PSSessionOption` 變數包含 **PSSessionOption** 物件，其中包含所有選項的預設值，如下所示。</span><span class="sxs-lookup"><span data-stu-id="0809c-440">By default, the `$PSSessionOption` variable contains a **PSSessionOption** object with the default values for all options, as shown below.</span></span>
...
<span data-ttu-id="0809c-441">如需這些選項的說明和詳細資訊，請參閱 [PSSessionOption](xref:Microsoft.PowerShell.Core.New-PSSessionOption)。</span><span class="sxs-lookup"><span data-stu-id="0809c-441">For descriptions of these options and more information, see [New-PSSessionOption](xref:Microsoft.PowerShell.Core.New-PSSessionOption).</span></span> <span data-ttu-id="0809c-442">如需遠端命令和會話的詳細資訊，請參閱 [about_Remote](about_Remote.md) 和 [about_PSSessions](about_PSSessions.md)。</span><span class="sxs-lookup"><span data-stu-id="0809c-442">For more information about remote commands and sessions, see [about_Remote](about_Remote.md) and [about_PSSessions](about_PSSessions.md).</span></span>

<span data-ttu-id="0809c-443">若要變更喜好設定變數的值 `$PSSessionOption` ，請使用 `New-PSSessionOption` Cmdlet 以您偏好的選項值來建立 **PSSessionOption** 物件。</span><span class="sxs-lookup"><span data-stu-id="0809c-443">To change the value of the `$PSSessionOption` preference variable, use the `New-PSSessionOption` cmdlet to create a **PSSessionOption** object with the option values you prefer.</span></span> <span data-ttu-id="0809c-444">將輸出儲存在名為的變數中 `$PSSessionOption` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-444">Save the output in a variable called `$PSSessionOption`.</span></span>

```powershell
$PSSessionOption = New-PSSessionOption -NoCompression
```

<span data-ttu-id="0809c-445">若要 `$PSSessionOption` 在每個 powershell 會話中使用喜好設定變數，請將 `New-PSSessionOption` 建立變數的命令新增 `$PSSessionOption` 至您的 powershell 設定檔。</span><span class="sxs-lookup"><span data-stu-id="0809c-445">To use the `$PSSessionOption` preference variable in every PowerShell session, add a `New-PSSessionOption` command that creates the `$PSSessionOption` variable to your PowerShell profile.</span></span> <span data-ttu-id="0809c-446">如需詳細資訊，請參閱 [about_Profiles](about_Profiles.md)。</span><span class="sxs-lookup"><span data-stu-id="0809c-446">For more information, see [about_Profiles](about_Profiles.md).</span></span>

<span data-ttu-id="0809c-447">您可以為特定的遠端會話設定自訂選項。</span><span class="sxs-lookup"><span data-stu-id="0809c-447">You can set custom options for a particular remote session.</span></span> <span data-ttu-id="0809c-448">您所設定的選項會優先于系統預設值和喜好設定變數的值 `$PSSessionOption` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-448">The options that you set take precedence over the system defaults and the value of the `$PSSessionOption` preference variable.</span></span>

<span data-ttu-id="0809c-449">若要設定自訂會話選項，請使用 `New-PSSessionOption` Cmdlet 來建立 **PSSessionOption** 物件。</span><span class="sxs-lookup"><span data-stu-id="0809c-449">To set custom session options, use the `New-PSSessionOption` cmdlet to create a **PSSessionOption** object.</span></span> <span data-ttu-id="0809c-450">然後，在建立會話的 Cmdlet 中使用 **PSSessionOption** 物件做為 **SessionOption** 參數的值，例如 `New-PSSession` 、 `Enter-PSSession` 和 `Invoke-Command` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-450">Then, use the **PSSessionOption** object as the value of the **SessionOption** parameter in cmdlets that create a session, such as `New-PSSession`, `Enter-PSSession`, and `Invoke-Command`.</span></span>

### <a name="transcript"></a><span data-ttu-id="0809c-451">$Transcript</span><span class="sxs-lookup"><span data-stu-id="0809c-451">$Transcript</span></span>

<span data-ttu-id="0809c-452">用 `Start-Transcript` 來指定文字記錄檔的名稱和位置。</span><span class="sxs-lookup"><span data-stu-id="0809c-452">Used by `Start-Transcript` to specify the name and location of the transcript file.</span></span> <span data-ttu-id="0809c-453">如果您未指定 **path** 參數的值，會 `Start-Transcript` 使用全域變數值中的路徑 `$Transcript` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-453">If you do not specify a value for the **Path** parameter, `Start-Transcript` uses the path in the value of the `$Transcript` global variable.</span></span> <span data-ttu-id="0809c-454">如果您尚未建立此變數，請將文字記錄以檔案的 `Start-Transcript` 形式儲存在 `$Home\My Documents` 目錄中 `\PowerShell_transcript.<time-stamp>.txt` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-454">If you have not created this variable, `Start-Transcript` stores the transcripts in the `$Home\My Documents` directory as `\PowerShell_transcript.<time-stamp>.txt` files.</span></span>

### <a name="verbosepreference"></a><span data-ttu-id="0809c-455">\$VerbosePreference</span><span class="sxs-lookup"><span data-stu-id="0809c-455">\$VerbosePreference</span></span>

<span data-ttu-id="0809c-456">決定 PowerShell 如何回應腳本、Cmdlet 或提供者所產生的詳細資訊訊息，例如 [寫入詳細](xref:Microsoft.PowerShell.Utility.Write-Verbose) 資訊 Cmdlet 所產生的訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-456">Determines how PowerShell responds to verbose messages generated by a script, cmdlet, or provider, such as the messages generated by the [Write-Verbose](xref:Microsoft.PowerShell.Utility.Write-Verbose) cmdlet.</span></span>
<span data-ttu-id="0809c-457">詳細資訊訊息說明執行命令所執行的動作。</span><span class="sxs-lookup"><span data-stu-id="0809c-457">Verbose messages describe the actions performed to execute a command.</span></span>

<span data-ttu-id="0809c-458">依預設不會顯示詳細訊息，但您可以藉由變更的值來變更此行為 `$VerbosePreference` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-458">By default, verbose messages aren't displayed, but you can change this behavior by changing the value of `$VerbosePreference`.</span></span>

<span data-ttu-id="0809c-459">您可以使用 Cmdlet 的 **verbose** 一般參數來顯示或隱藏特定命令的詳細資訊訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-459">You can use the **Verbose** common parameter of a cmdlet to display or hide the verbose messages for a specific command.</span></span> <span data-ttu-id="0809c-460">如需詳細資訊，請參閱 [about_CommonParameters](about_CommonParameters.md)。</span><span class="sxs-lookup"><span data-stu-id="0809c-460">For more information, see [about_CommonParameters](about_CommonParameters.md).</span></span>

<span data-ttu-id="0809c-461">有效值如下：</span><span class="sxs-lookup"><span data-stu-id="0809c-461">The valid values are as follows:</span></span>

- <span data-ttu-id="0809c-462">**停止** ：顯示詳細資訊訊息和錯誤訊息，然後停止執行。</span><span class="sxs-lookup"><span data-stu-id="0809c-462">**Stop** : Displays the verbose message and an error message and then stops executing.</span></span>
- <span data-ttu-id="0809c-463">**Inquire** ：顯示詳細資訊訊息，然後顯示詢問您是否要繼續的提示。</span><span class="sxs-lookup"><span data-stu-id="0809c-463">**Inquire** : Displays the verbose message and then displays a prompt that asks you whether you want to continue.</span></span>
- <span data-ttu-id="0809c-464">**繼續** ：顯示詳細資訊訊息，然後繼續執行。</span><span class="sxs-lookup"><span data-stu-id="0809c-464">**Continue** : Displays the verbose message and then continues with execution.</span></span>
- <span data-ttu-id="0809c-465">**SilentlyContinue** ： (預設) 不會顯示詳細資訊訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-465">**SilentlyContinue** : (Default) Doesn't display the verbose message.</span></span> <span data-ttu-id="0809c-466">繼續執行。</span><span class="sxs-lookup"><span data-stu-id="0809c-466">Continues executing.</span></span>

#### <a name="examples"></a><span data-ttu-id="0809c-467">範例</span><span class="sxs-lookup"><span data-stu-id="0809c-467">Examples</span></span>

<span data-ttu-id="0809c-468">這些範例會顯示不同值的效果 `$VerbosePreference` ，以及 **詳細** 資訊參數以覆寫喜好設定值。</span><span class="sxs-lookup"><span data-stu-id="0809c-468">These examples show the effect of the different values of `$VerbosePreference` and the **Verbose** parameter to override the preference value.</span></span>

<span data-ttu-id="0809c-469">此範例顯示 **SilentlyContinue** 值的效果，這是預設值。</span><span class="sxs-lookup"><span data-stu-id="0809c-469">This example shows the effect of the **SilentlyContinue** value, that is the default.</span></span> <span data-ttu-id="0809c-470">此命令會使用 **message** 參數，但不會將訊息寫入 PowerShell 主控台。</span><span class="sxs-lookup"><span data-stu-id="0809c-470">The command uses the **Message** parameter, but doesn't write a message to the PowerShell console.</span></span>

```powershell
Write-Verbose -Message "Verbose message test."
```

<span data-ttu-id="0809c-471">使用 **Verbose** 參數時，會寫入訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-471">When the **Verbose** parameter is used, the message is written.</span></span>
...
<span data-ttu-id="0809c-472">此範例顯示 **Continue** 值的效果。</span><span class="sxs-lookup"><span data-stu-id="0809c-472">This example shows the effect of the **Continue** value.</span></span> <span data-ttu-id="0809c-473">`$VerbosePreference`變數設定為 **Continue** ，並顯示訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-473">The `$VerbosePreference` variable is set to **Continue** and the message is displayed.</span></span>
...
<span data-ttu-id="0809c-474">這個範例會使用 **Verbose** 參數，其值 `$false` 會覆寫 **Continue** 值。</span><span class="sxs-lookup"><span data-stu-id="0809c-474">This example uses the **Verbose** parameter with a value of `$false` that overrides the **Continue** value.</span></span> <span data-ttu-id="0809c-475">不會顯示訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-475">The message isn't displayed.</span></span>

```powershell
Write-Verbose -Message "Verbose message test." -Verbose:$false
```

<span data-ttu-id="0809c-476">此範例顯示 **Stop** 值的效果。</span><span class="sxs-lookup"><span data-stu-id="0809c-476">This example shows the effect of the **Stop** value.</span></span> <span data-ttu-id="0809c-477">`$VerbosePreference`變數已設定為 [ **停止** ]，並顯示訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-477">The `$VerbosePreference` variable is set to **Stop** and the message is displayed.</span></span> <span data-ttu-id="0809c-478">此命令已停止。</span><span class="sxs-lookup"><span data-stu-id="0809c-478">The command is stopped.</span></span>
...
<span data-ttu-id="0809c-479">這個範例會使用 **Verbose** 參數，其值 `$false` 會覆寫 **停止** 值。</span><span class="sxs-lookup"><span data-stu-id="0809c-479">This example uses the **Verbose** parameter with a value of `$false` that overrides the **Stop** value.</span></span> <span data-ttu-id="0809c-480">不會顯示訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-480">The message isn't displayed.</span></span>

```powershell
Write-Verbose -Message "Verbose message test." -Verbose:$false
```

<span data-ttu-id="0809c-481">此範例顯示 **Inquire** 值的效果。</span><span class="sxs-lookup"><span data-stu-id="0809c-481">This example shows the effect of the **Inquire** value.</span></span> <span data-ttu-id="0809c-482">`$VerbosePreference`變數設定為 **Inquire** 。</span><span class="sxs-lookup"><span data-stu-id="0809c-482">The `$VerbosePreference` variable is set to **Inquire** .</span></span> <span data-ttu-id="0809c-483">訊息隨即顯示，並提示使用者進行確認。</span><span class="sxs-lookup"><span data-stu-id="0809c-483">The message is displayed and the user is prompted for confirmation.</span></span>
...
<span data-ttu-id="0809c-484">這個範例會使用 **Verbose** 參數，其值 `$false` 會覆寫 **查詢** 值。</span><span class="sxs-lookup"><span data-stu-id="0809c-484">This example uses the **Verbose** parameter with a value of `$false` that overrides the **Inquire** value.</span></span> <span data-ttu-id="0809c-485">系統不會提示使用者，也不會顯示訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-485">The user isn't prompted and the message isn't displayed.</span></span>

```powershell
Write-Verbose -Message "Verbose message test." -Verbose:$false
```

### <a name="warningpreference"></a><span data-ttu-id="0809c-486">\$WarningPreference</span><span class="sxs-lookup"><span data-stu-id="0809c-486">\$WarningPreference</span></span>

<span data-ttu-id="0809c-487">決定 PowerShell 如何回應腳本、Cmdlet 或提供者所產生的警告訊息，例如由 [Write-warning](xref:Microsoft.PowerShell.Utility.Write-Warning) Cmdlet 所產生的訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-487">Determines how PowerShell responds to warning messages generated by a script, cmdlet, or provider, such as the messages generated by the [Write-Warning](xref:Microsoft.PowerShell.Utility.Write-Warning) cmdlet.</span></span>

<span data-ttu-id="0809c-488">根據預設，會顯示警告訊息並繼續執行，但您可以藉由變更的值來變更此行為 `$WarningPreference` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-488">By default, warning messages are displayed and execution continues, but you can change this behavior by changing the value of `$WarningPreference`.</span></span>

<span data-ttu-id="0809c-489">您可以使用 Cmdlet 的 **WarningAction** 一般參數，來判斷 PowerShell 如何回應特定命令的警告。</span><span class="sxs-lookup"><span data-stu-id="0809c-489">You can use the **WarningAction** common parameter of a cmdlet to determine how PowerShell responds to warnings from a particular command.</span></span> <span data-ttu-id="0809c-490">如需詳細資訊，請參閱 [about_CommonParameters](about_CommonParameters.md)。</span><span class="sxs-lookup"><span data-stu-id="0809c-490">For more information, see [about_CommonParameters](about_CommonParameters.md).</span></span>

<span data-ttu-id="0809c-491">有效值如下：</span><span class="sxs-lookup"><span data-stu-id="0809c-491">The valid values are as follows:</span></span>

- <span data-ttu-id="0809c-492">**停止** ：顯示警告訊息和錯誤訊息，然後停止執行。</span><span class="sxs-lookup"><span data-stu-id="0809c-492">**Stop** : Displays the warning message and an error message and then stops executing.</span></span>
- <span data-ttu-id="0809c-493">**查詢** ：顯示警告訊息，然後提示您取得繼續的許可權。</span><span class="sxs-lookup"><span data-stu-id="0809c-493">**Inquire** : Displays the warning message and then prompts for permission to continue.</span></span>
- <span data-ttu-id="0809c-494">**繼續** ： (預設值) 顯示警告訊息，然後繼續執行。</span><span class="sxs-lookup"><span data-stu-id="0809c-494">**Continue** : (Default) Displays the warning message and then continues executing.</span></span>
- <span data-ttu-id="0809c-495">**SilentlyContinue** ：不顯示警告訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-495">**SilentlyContinue** : Doesn't display the warning message.</span></span> <span data-ttu-id="0809c-496">繼續執行。</span><span class="sxs-lookup"><span data-stu-id="0809c-496">Continues executing.</span></span>

#### <a name="examples"></a><span data-ttu-id="0809c-497">範例</span><span class="sxs-lookup"><span data-stu-id="0809c-497">Examples</span></span>

<span data-ttu-id="0809c-498">這些範例會顯示不同值的效果 `$WarningPreference` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-498">These examples show the effect of the different values of `$WarningPreference`.</span></span>
<span data-ttu-id="0809c-499">**WarningAction** 參數會覆寫喜好設定值。</span><span class="sxs-lookup"><span data-stu-id="0809c-499">The **WarningAction** parameter overrides the preference value.</span></span>

<span data-ttu-id="0809c-500">此範例顯示預設值的效果 [ **Continue** ]。</span><span class="sxs-lookup"><span data-stu-id="0809c-500">This example shows the effect of the default value, **Continue** .</span></span>
...
<span data-ttu-id="0809c-501">此範例使用 **WarningAction** 參數搭配值 **SilentlyContinue** 來抑制警告。</span><span class="sxs-lookup"><span data-stu-id="0809c-501">This example uses the **WarningAction** parameter with the value **SilentlyContinue** to suppress the warning.</span></span> <span data-ttu-id="0809c-502">不會顯示訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-502">The message isn't displayed.</span></span>
...
<span data-ttu-id="0809c-503">此範例會將 `$WarningPreference` 變數變更為 **SilentlyContinue** 值。</span><span class="sxs-lookup"><span data-stu-id="0809c-503">This example changes the `$WarningPreference` variable to the **SilentlyContinue** value.</span></span> <span data-ttu-id="0809c-504">不會顯示訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-504">The message isn't displayed.</span></span>
...
<span data-ttu-id="0809c-505">此範例會在產生警告時使用 **WarningAction** 參數來停止。</span><span class="sxs-lookup"><span data-stu-id="0809c-505">This example uses the **WarningAction** parameter to stop when a warning is generated.</span></span>
...
<span data-ttu-id="0809c-506">此範例會將 `$WarningPreference` 變數變更為 **查詢** 值。</span><span class="sxs-lookup"><span data-stu-id="0809c-506">This example changes the `$WarningPreference` variable to the **Inquire** value.</span></span> <span data-ttu-id="0809c-507">系統會提示使用者進行確認。</span><span class="sxs-lookup"><span data-stu-id="0809c-507">The user is prompted for confirmation.</span></span>
...
<span data-ttu-id="0809c-508">此範例使用 **WarningAction** 參數搭配值 **SilentlyContinue** 。</span><span class="sxs-lookup"><span data-stu-id="0809c-508">This example uses the **WarningAction** parameter with the value **SilentlyContinue** .</span></span> <span data-ttu-id="0809c-509">此命令會繼續執行，且不會顯示任何訊息。</span><span class="sxs-lookup"><span data-stu-id="0809c-509">The command continues to execute and no message is displayed.</span></span>
...
<span data-ttu-id="0809c-510">此範例會將 `$WarningPreference` 值變更為 [ **停止** ]。</span><span class="sxs-lookup"><span data-stu-id="0809c-510">This example changes the `$WarningPreference` value to **Stop** .</span></span>
...
<span data-ttu-id="0809c-511">此範例會使用 **WarningAction** 搭配 **查詢** 值。</span><span class="sxs-lookup"><span data-stu-id="0809c-511">This example uses the **WarningAction** with the **Inquire** value.</span></span> <span data-ttu-id="0809c-512">出現警告時，系統會提示使用者。</span><span class="sxs-lookup"><span data-stu-id="0809c-512">The user is prompted when a warning occurs.</span></span>
...
### <a name="whatifpreference"></a><span data-ttu-id="0809c-513">\$WhatIfPreference</span><span class="sxs-lookup"><span data-stu-id="0809c-513">\$WhatIfPreference</span></span>

<span data-ttu-id="0809c-514">決定是否針對支援的每個命令自動啟用 **WhatIf** 。</span><span class="sxs-lookup"><span data-stu-id="0809c-514">Determines whether **WhatIf** is automatically enabled for every command that supports it.</span></span> <span data-ttu-id="0809c-515">當 **WhatIf** 啟用時，此 Cmdlet 會報告命令的預期效果，但不會執行命令。</span><span class="sxs-lookup"><span data-stu-id="0809c-515">When **WhatIf** is enabled, the cmdlet reports the expected effect of the command, but doesn't execute the command.</span></span>

<span data-ttu-id="0809c-516">有效值如下：</span><span class="sxs-lookup"><span data-stu-id="0809c-516">The valid values are as follows:</span></span>

- <span data-ttu-id="0809c-517">**False** ( **0** ，未啟用) ： (預設) **WhatIf** 不會自動啟用。</span><span class="sxs-lookup"><span data-stu-id="0809c-517">**False** ( **0** , not enabled): (Default) **WhatIf** isn't automatically enabled.</span></span> <span data-ttu-id="0809c-518">若要手動啟用，請使用 Cmdlet 的 **WhatIf** 參數。</span><span class="sxs-lookup"><span data-stu-id="0809c-518">To enable it manually, use the cmdlet's **WhatIf** parameter.</span></span>
- <span data-ttu-id="0809c-519">**True** ( **1** ，enabled) ： **WhatIf** 會在任何支援的命令上自動啟用。</span><span class="sxs-lookup"><span data-stu-id="0809c-519">**True** ( **1** , enabled): **WhatIf** is automatically enabled on any command that supports it.</span></span> <span data-ttu-id="0809c-520">使用者可以將 **WhatIf** 參數的值設為 **False** ，以手動方式停用它，例如 `-WhatIf:$false` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-520">Users can use the **WhatIf** parameter with a value of **False** to disable it manually, such as `-WhatIf:$false`.</span></span>

#### <a name="examples"></a><span data-ttu-id="0809c-521">範例</span><span class="sxs-lookup"><span data-stu-id="0809c-521">Examples</span></span>

<span data-ttu-id="0809c-522">這些範例會顯示不同值的效果 `$WhatIfPreference` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-522">These examples show the effect of the different values of `$WhatIfPreference`.</span></span>
<span data-ttu-id="0809c-523">它們會示範如何使用 **WhatIf** 參數來覆寫特定命令的喜好設定值。</span><span class="sxs-lookup"><span data-stu-id="0809c-523">They show how to use the **WhatIf** parameter to override the preference value for a specific command.</span></span>

<span data-ttu-id="0809c-524">此範例顯示 `$WhatIfPreference` 變數設定為預設值 **False** 的效果。</span><span class="sxs-lookup"><span data-stu-id="0809c-524">This example shows the effect of the `$WhatIfPreference` variable set to the default value, **False** .</span></span> <span data-ttu-id="0809c-525">使用 `Get-ChildItem` 確認檔案是否存在。</span><span class="sxs-lookup"><span data-stu-id="0809c-525">Use `Get-ChildItem` to verify that the file exists.</span></span>
<span data-ttu-id="0809c-526">`Remove-Item` 刪除檔案。</span><span class="sxs-lookup"><span data-stu-id="0809c-526">`Remove-Item` deletes the file.</span></span> <span data-ttu-id="0809c-527">刪除檔案之後，您可以使用確認刪除 `Get-ChildItem` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-527">After the file is deleted, you can verify the deletion with `Get-ChildItem`.</span></span>
...
<span data-ttu-id="0809c-528">此範例顯示當的值為 False 時，使用 **WhatIf** 參數的 `$WhatIfPreference` 效果 **False** 。</span><span class="sxs-lookup"><span data-stu-id="0809c-528">This example shows the effect of using the **WhatIf** parameter when the value of `$WhatIfPreference` is **False** .</span></span>

<span data-ttu-id="0809c-529">請確認檔案存在。</span><span class="sxs-lookup"><span data-stu-id="0809c-529">Verify that the file exists.</span></span>
...
<span data-ttu-id="0809c-530">使用 **WhatIf** 參數來判斷嘗試刪除檔案的結果。</span><span class="sxs-lookup"><span data-stu-id="0809c-530">Use the **WhatIf** parameter to determine the result of attempting to delete the file.</span></span>
...
<span data-ttu-id="0809c-531">確認未刪除該檔案。</span><span class="sxs-lookup"><span data-stu-id="0809c-531">Verify that the file wasn't deleted.</span></span>
...
<span data-ttu-id="0809c-532">此範例顯示 `$WhatIfPreference` 變數設定為 **True** 的效果。</span><span class="sxs-lookup"><span data-stu-id="0809c-532">This example shows the effect of the `$WhatIfPreference` variable set to the value, **True** .</span></span> <span data-ttu-id="0809c-533">當您使用刪除檔案時， `Remove-Item` 會顯示檔案的路徑，但不會刪除檔案。</span><span class="sxs-lookup"><span data-stu-id="0809c-533">When you use `Remove-Item` to delete a file, the file's path is displayed, but the file isn't deleted.</span></span>

<span data-ttu-id="0809c-534">嘗試刪除檔案。</span><span class="sxs-lookup"><span data-stu-id="0809c-534">Attempt to delete a file.</span></span> <span data-ttu-id="0809c-535">當執行時，會顯示一則訊息 `Remove-Item` ，但不會刪除該檔案。</span><span class="sxs-lookup"><span data-stu-id="0809c-535">A message is displayed about what would happen if `Remove-Item` was run, but the file isn't deleted.</span></span>
...
<span data-ttu-id="0809c-536">使用 `Get-ChildItem` 確認未刪除檔案。</span><span class="sxs-lookup"><span data-stu-id="0809c-536">Use `Get-ChildItem` to verify that the file wasn't deleted.</span></span>
...
<span data-ttu-id="0809c-537">這個範例示範當的值為 True 時，如何刪除 `$WhatIfPreference` 檔案 **True** 。</span><span class="sxs-lookup"><span data-stu-id="0809c-537">This example shows how to delete a file when the value of `$WhatIfPreference` is **True** .</span></span> <span data-ttu-id="0809c-538">它會使用具有值的 **WhatIf** 參數 `$false` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-538">It uses the **WhatIf** parameter with a value of `$false`.</span></span> <span data-ttu-id="0809c-539">使用 `Get-ChildItem` 確認檔案已刪除。</span><span class="sxs-lookup"><span data-stu-id="0809c-539">Use `Get-ChildItem` to verify the file was deleted.</span></span>
...
<span data-ttu-id="0809c-540">以下是不 `Get-Process` 支援 **whatif** 且 `Stop-Process` 支援 **whatif** 的 Cmdlet 範例。</span><span class="sxs-lookup"><span data-stu-id="0809c-540">The following are examples of the `Get-Process` cmdlet that doesn't support **WhatIf** and `Stop-Process` that does support **WhatIf** .</span></span> <span data-ttu-id="0809c-541">`$WhatIfPreference`變數的值為 **True** 。</span><span class="sxs-lookup"><span data-stu-id="0809c-541">The `$WhatIfPreference` variable's value is **True** .</span></span>

<span data-ttu-id="0809c-542">`Get-Process` 不支援 **WhatIf** 。</span><span class="sxs-lookup"><span data-stu-id="0809c-542">`Get-Process` doesn't support **WhatIf** .</span></span> <span data-ttu-id="0809c-543">當命令執行時，它會顯示 **Winword** 處理常式。</span><span class="sxs-lookup"><span data-stu-id="0809c-543">When the command is executed, it displays the **Winword** process.</span></span>
...
<span data-ttu-id="0809c-544">`Stop-Process` 支援 **WhatIf** 。</span><span class="sxs-lookup"><span data-stu-id="0809c-544">`Stop-Process` does support **WhatIf** .</span></span> <span data-ttu-id="0809c-545">**Winword** 進程不會停止。</span><span class="sxs-lookup"><span data-stu-id="0809c-545">The **Winword** process isn't stopped.</span></span>
...
<span data-ttu-id="0809c-546">您可以使用 Whatif 參數搭配的值來覆寫 `Stop-Process` **whatif** 行為 **WhatIf** `$false` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-546">You can override the `Stop-Process` **WhatIf** behavior by using the **WhatIf** parameter with a value of `$false`.</span></span> <span data-ttu-id="0809c-547">**Winword** 進程已停止。</span><span class="sxs-lookup"><span data-stu-id="0809c-547">The **Winword** process is stopped.</span></span>

```powershell
Stop-Process -Name Winword -WhatIf:$false
```

<span data-ttu-id="0809c-548">若要確認 **Winword** 進程已停止，請使用 `Get-Process` 。</span><span class="sxs-lookup"><span data-stu-id="0809c-548">To verify that the **Winword** process was stopped, use `Get-Process`.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="0809c-549">另請參閱</span><span class="sxs-lookup"><span data-stu-id="0809c-549">See also</span></span>

[<span data-ttu-id="0809c-550">about_Automatic_Variables</span><span class="sxs-lookup"><span data-stu-id="0809c-550">about_Automatic_Variables</span></span>](about_Automatic_Variables.md)

[<span data-ttu-id="0809c-551">about_CommonParameters</span><span class="sxs-lookup"><span data-stu-id="0809c-551">about_CommonParameters</span></span>](about_CommonParameters.md)

[<span data-ttu-id="0809c-552">about_Environment_Variables</span><span class="sxs-lookup"><span data-stu-id="0809c-552">about_Environment_Variables</span></span>](about_Environment_Variables.md)

[<span data-ttu-id="0809c-553">about_Profiles</span><span class="sxs-lookup"><span data-stu-id="0809c-553">about_Profiles</span></span>](about_Profiles.md)

[<span data-ttu-id="0809c-554">about_Remote</span><span class="sxs-lookup"><span data-stu-id="0809c-554">about_Remote</span></span>](about_Remote.md)

[<span data-ttu-id="0809c-555">about_Scopes</span><span class="sxs-lookup"><span data-stu-id="0809c-555">about_Scopes</span></span>](about_Scopes.md)

[<span data-ttu-id="0809c-556">about_Variables</span><span class="sxs-lookup"><span data-stu-id="0809c-556">about_Variables</span></span>](about_Variables.md)
