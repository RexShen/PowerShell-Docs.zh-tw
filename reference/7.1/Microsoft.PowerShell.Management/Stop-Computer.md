...
在 PowerShell 7.1 中， `Stop-Computer` 已為 Linux 和 macOS 新增。 這些參數在這些平臺上不會有任何作用。 此 Cmdlet 只會呼叫原生命令 `/sbin/shutdown` 。
...
在 PowerShell 7.1 中， `Stop-Computer` 已為 Linux 和 macOS 新增。 針對這些平臺，此 Cmdlet 會呼叫原生命令 `/sbin/shutdown` 。
