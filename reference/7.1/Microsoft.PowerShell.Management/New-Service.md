...
### 範例4：在建立時設定服務的 SecurityDescriptor。

此範例會新增要建立之服務的 **SecurityDescriptor** 。
...
**SecurityDescriptor** 會儲存在變數中 `$SDDLToSet` 。 **SecurityDescriptorSddl** 參數會使用 `$SDDL` 來設定新服務的 **SecurityDescriptor** 。
