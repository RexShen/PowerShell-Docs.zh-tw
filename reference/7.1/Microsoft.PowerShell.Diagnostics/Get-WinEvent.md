...
### 範例18：使用 SuppressHashFilter 來篩選應用程式錯誤

如同上述的範例16，此範例使用 **FilterHashtable** 參數從 **應用程式** 記錄檔取得事件。 不過，我們會加入 **SuppressHashFilter** 索引鍵，以篩選出 **資訊** 層級事件。
...
在此範例中， `Get-WinEvent` 會取得過去兩天內 **應用程式** 記錄檔中的所有事件，但其 **層級** 為 4 (資訊) 。
...
您也可以使用 **FilterHashtable** 參數建立 **隱藏** 元素。
