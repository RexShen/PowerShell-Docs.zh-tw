...
### <span data-ttu-id="cab5b-109">工作階段</span><span class="sxs-lookup"><span data-stu-id="cab5b-109">Session</span></span>
...
### <span data-ttu-id="cab5b-119">ContainerId</span><span class="sxs-lookup"><span data-stu-id="cab5b-119">ContainerId</span></span>
...
### <span data-ttu-id="cab5b-142">範例1：在伺服器上執行腳本</span><span class="sxs-lookup"><span data-stu-id="cab5b-142">Example 1: Run a script on a server</span></span>
...
<span data-ttu-id="cab5b-169">問題的解決方式是在遠端電腦上建立持續性會話，然後在相同的會話中執行這兩個命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-169">The problem is solved by creating a persistent session on the remote computer, then running both of the commands in the same session.</span></span>
...
### <span data-ttu-id="cab5b-184">範例7：在多部電腦上取得主機程式的版本</span><span class="sxs-lookup"><span data-stu-id="cab5b-184">Example 7: Get the version of the host program on multiple computers</span></span>
...
### <span data-ttu-id="cab5b-207">範例9：在遠端電腦上執行的命令中包含本機變數</span><span class="sxs-lookup"><span data-stu-id="cab5b-207">Example 9: Include local variables in a command run on a remote computer</span></span>
...
### <span data-ttu-id="cab5b-231">範例12：在腳本區塊中使用 $args 自動變數</span><span class="sxs-lookup"><span data-stu-id="cab5b-231">Example 12: Use the $args automatic variable in a script block</span></span>
...
### <span data-ttu-id="cab5b-255">範例15：使用會話選項</span><span class="sxs-lookup"><span data-stu-id="cab5b-255">Example 15: Use a session option</span></span>
...
<span data-ttu-id="cab5b-266">此 `Invoke-Command` Cmdlet 會 `Get-Mailbox` 在遠端 Microsoft Exchange 伺服器上執行命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-266">The `Invoke-Command` cmdlet runs a `Get-Mailbox` command on a remote Microsoft Exchange Server.</span></span> <span data-ttu-id="cab5b-267">**AllowRedirection** 參數提供明確的許可權，以將連接重新導向至替代端點。</span><span class="sxs-lookup"><span data-stu-id="cab5b-267">The **AllowRedirection** parameter provides explicit permission to redirect the connection to an alternate endpoint.</span></span> <span data-ttu-id="cab5b-268">**SessionOption** 參數會使用儲存在變數中的會話物件 `$max` 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-268">The **SessionOption** parameter uses the session object stored in the `$max` variable.</span></span>
...
### <span data-ttu-id="cab5b-306">範例21：在多部遠端電腦上使用 SSH 作為作業來執行腳本檔案</span><span class="sxs-lookup"><span data-stu-id="cab5b-306">Example 21: Run a script file on multiple remote computers using SSH as a job</span></span>
...
### <span data-ttu-id="cab5b-318">-ApplicationName</span><span class="sxs-lookup"><span data-stu-id="cab5b-318">-ApplicationName</span></span>
...
<span data-ttu-id="cab5b-333">**ArgumentList** 參數中的值可以是實際值（例如1024），也可以是區域變數（例如）的參考 `$max` 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-333">The values in the **ArgumentList** parameter can be actual values, such as 1024, or they can be references to local variables, such as `$max`.</span></span>
...
<span data-ttu-id="cab5b-361">預設值為 Default。</span><span class="sxs-lookup"><span data-stu-id="cab5b-361">The default value is Default.</span></span>
...
<span data-ttu-id="cab5b-372">若要取得憑證指紋，請使用 `Get-Item` `Get-ChildItem` PowerShell Cert：磁片磁碟機中的或命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-372">To get a certificate thumbprint, use a `Get-Item` or `Get-ChildItem` command in the PowerShell Cert: drive.</span></span>
...
<span data-ttu-id="cab5b-396">指定定義工作階段之連線端點的「統一資源識別項」(URI)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-396">Specifies a Uniform Resource Identifier (URI) that defines the connection endpoint of the session.</span></span>
<span data-ttu-id="cab5b-397">此 URI 必須是完整的 URI。</span><span class="sxs-lookup"><span data-stu-id="cab5b-397">The URI must be fully qualified.</span></span>

<span data-ttu-id="cab5b-398">這個字串的格式如下所示：</span><span class="sxs-lookup"><span data-stu-id="cab5b-398">The format of this string is as follows:</span></span>

`<Transport>://<ComputerName>:<Port>/<ApplicationName>`

<span data-ttu-id="cab5b-399">預設值如下：</span><span class="sxs-lookup"><span data-stu-id="cab5b-399">The default value is as follows:</span></span>

`http://localhost:5985/WSMAN`

<span data-ttu-id="cab5b-400">如果您未指定連接 URI，您可以使用 **UseSSL** 和 **Port** 參數來指定連接 uri 值。</span><span class="sxs-lookup"><span data-stu-id="cab5b-400">If you don't specify a connection URI, you can use the **UseSSL** and **Port** parameters to specify the connection URI values.</span></span>

<span data-ttu-id="cab5b-401">URI 的 **Transport** 區段有效值為 HTTP 與 HTTPS。</span><span class="sxs-lookup"><span data-stu-id="cab5b-401">Valid values for the **Transport** segment of the URI are HTTP and HTTPS.</span></span> <span data-ttu-id="cab5b-402">如果您指定具有傳輸區段的連線 URI，但未指定埠，則會使用標準埠：80（適用于 HTTP）和443（適用于 HTTPS）來建立會話。</span><span class="sxs-lookup"><span data-stu-id="cab5b-402">If you specify a connection URI with a Transport segment, but don't specify a port, the session is created with the standards ports: 80 for HTTP and 443 for HTTPS.</span></span> <span data-ttu-id="cab5b-403">若要使用 PowerShell 遠端的預設埠，請指定 HTTP 的埠5985或 HTTPS 的5986。</span><span class="sxs-lookup"><span data-stu-id="cab5b-403">To use the default ports for PowerShell remoting, specify port 5985 for HTTP or 5986 for HTTPS.</span></span>

<span data-ttu-id="cab5b-404">如果目的地電腦將連線重新導向至不同的 URI，除非您在命令中使用 **AllowRedirection** 參數，否則 PowerShell 會禁止重新導向。</span><span class="sxs-lookup"><span data-stu-id="cab5b-404">If the destination computer redirects the connection to a different URI, PowerShell prevents the redirection unless you use the **AllowRedirection** parameter in the command.</span></span>
...
### <span data-ttu-id="cab5b-405">-ContainerId</span><span class="sxs-lookup"><span data-stu-id="cab5b-405">-ContainerId</span></span>

<span data-ttu-id="cab5b-406">指定容器識別碼的陣列。</span><span class="sxs-lookup"><span data-stu-id="cab5b-406">Specifies an array of container IDs.</span></span>
...
### <span data-ttu-id="cab5b-407">-Credential</span><span class="sxs-lookup"><span data-stu-id="cab5b-407">-Credential</span></span>

<span data-ttu-id="cab5b-408">指定具有執行此動作權限的使用者帳戶。</span><span class="sxs-lookup"><span data-stu-id="cab5b-408">Specifies a user account that has permission to perform this action.</span></span> <span data-ttu-id="cab5b-409">預設為目前使用者。</span><span class="sxs-lookup"><span data-stu-id="cab5b-409">The default is the current user.</span></span>

<span data-ttu-id="cab5b-410">輸入使用者名稱（例如 **User01** 或 **Domain01\User01** ），或輸入 Cmdlet 所產生的 **PSCredential** 物件 `Get-Credential` 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-410">Type a user name, such as **User01** or **Domain01\User01** , or enter a **PSCredential** object generated by the `Get-Credential` cmdlet.</span></span> <span data-ttu-id="cab5b-411">如果您輸入使用者名稱，系統就會提示您輸入密碼。</span><span class="sxs-lookup"><span data-stu-id="cab5b-411">If you type a user name, you're prompted to enter the password.</span></span>

<span data-ttu-id="cab5b-412">認證會儲存在 [PSCredential](/dotnet/api/system.management.automation.pscredential) 物件中，而密碼會儲存為 [SecureString](/dotnet/api/system.security.securestring)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-412">Credentials are stored in a [PSCredential](/dotnet/api/system.management.automation.pscredential) object and the password is stored as a [SecureString](/dotnet/api/system.security.securestring).</span></span>

> [!NOTE]
> <span data-ttu-id="cab5b-413">如需有關 **SecureString** 資料保護的詳細資訊，請參閱 [如何 SecureString 安全？](/dotnet/api/system.security.securestring#how-secure-is-securestring)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-413">For more information about **SecureString** data protection, see [How secure is SecureString?](/dotnet/api/system.security.securestring#how-secure-is-securestring).</span></span>
...
### <span data-ttu-id="cab5b-414">-EnableNetworkAccess</span><span class="sxs-lookup"><span data-stu-id="cab5b-414">-EnableNetworkAccess</span></span>

<span data-ttu-id="cab5b-415">指出此 Cmdlet 會將互動式安全性權杖新增至回送會話。</span><span class="sxs-lookup"><span data-stu-id="cab5b-415">Indicates that this cmdlet adds an interactive security token to loopback sessions.</span></span> <span data-ttu-id="cab5b-416">互動式權杖可讓您在會從其他電腦取得資料的回送工作階段中執行命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-416">The interactive token lets you run commands in the loopback session that get data from other computers.</span></span> <span data-ttu-id="cab5b-417">例如，您可以在會將 XML 檔案從遠端電腦複製到本機電腦的工作階段中執行命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-417">For example, you can run a command in the session that copies XML files from a remote computer to the local computer.</span></span>

<span data-ttu-id="cab5b-418">回送會話是在同一部電腦上產生和結束的 **PSSession** 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-418">A loopback session is a **PSSession** that originates and ends on the same computer.</span></span> <span data-ttu-id="cab5b-419">若要建立回送會話，請省略 **ComputerName** 參數，或將其值設為點 (`.`) 、localhost 或本機電腦的名稱。</span><span class="sxs-lookup"><span data-stu-id="cab5b-419">To create a loopback session, omit the **ComputerName** parameter or set its value to dot (`.`), localhost, or the name of the local computer.</span></span>

<span data-ttu-id="cab5b-420">根據預設，系統會使用網路權杖建立回送會話，這可能不會提供足夠的許可權來驗證遠端電腦。</span><span class="sxs-lookup"><span data-stu-id="cab5b-420">By default, loopback sessions are created using a network token, which might not provide sufficient permission to authenticate to remote computers.</span></span>

<span data-ttu-id="cab5b-421">**EnableNetworkAccess** 參數只在回送工作階段中有效。</span><span class="sxs-lookup"><span data-stu-id="cab5b-421">The **EnableNetworkAccess** parameter is effective only in loopback sessions.</span></span> <span data-ttu-id="cab5b-422">如果您在遠端電腦上建立會話時使用 **EnableNetworkAccess** ，則命令會成功，但是會忽略參數。</span><span class="sxs-lookup"><span data-stu-id="cab5b-422">If you use **EnableNetworkAccess** when you create a session on a remote computer, the command succeeds, but the parameter is ignored.</span></span>

<span data-ttu-id="cab5b-423">您可以使用 **驗證** 參數的 **CredSSP** 值（將會話認證委派給其他電腦），允許回送會話中的遠端存取。</span><span class="sxs-lookup"><span data-stu-id="cab5b-423">You can allow remote access in a loopback session using the **CredSSP** value of the **Authentication** parameter, which delegates the session credentials to other computers.</span></span>

<span data-ttu-id="cab5b-424">為了保護電腦免于惡意存取，具有互動式權杖的已中斷連線回送會話（使用 **EnableNetworkAccess** 所建立的權杖）只能從建立會話的電腦重新連接。</span><span class="sxs-lookup"><span data-stu-id="cab5b-424">To protect the computer from malicious access, disconnected loopback sessions that have interactive tokens, which are those created using **EnableNetworkAccess** , can be reconnected only from the computer on which the session was created.</span></span> <span data-ttu-id="cab5b-425">使用 CredSSP 驗證且已中斷連線的工作階段可從其他電腦重新連線。</span><span class="sxs-lookup"><span data-stu-id="cab5b-425">Disconnected sessions that use CredSSP authentication can be reconnected from other computers.</span></span> <span data-ttu-id="cab5b-426">如需詳細資訊，請參閱`Disconnect-PSSession`。</span><span class="sxs-lookup"><span data-stu-id="cab5b-426">For more information, see `Disconnect-PSSession`.</span></span>

<span data-ttu-id="cab5b-427">此參數是在 PowerShell 3.0 中引進。</span><span class="sxs-lookup"><span data-stu-id="cab5b-427">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="cab5b-428">-FilePath</span><span class="sxs-lookup"><span data-stu-id="cab5b-428">-FilePath</span></span>

<span data-ttu-id="cab5b-429">指定此 Cmdlet 在一或多部遠端電腦上執行的本機腳本。</span><span class="sxs-lookup"><span data-stu-id="cab5b-429">Specifies a local script that this cmdlet runs on one or more remote computers.</span></span> <span data-ttu-id="cab5b-430">輸入腳本的路徑和檔案名，或使用管線將腳本路徑傳送至 `Invoke-Command` 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-430">Enter the path and filename of the script, or pipe a script path to `Invoke-Command`.</span></span> <span data-ttu-id="cab5b-431">腳本必須存在於本機電腦或本機電腦可以存取的目錄中。</span><span class="sxs-lookup"><span data-stu-id="cab5b-431">The script must exist on the local computer or in a directory that the local computer can access.</span></span> <span data-ttu-id="cab5b-432">使用 **ArgumentList** 來指定腳本中參數的值。</span><span class="sxs-lookup"><span data-stu-id="cab5b-432">Use **ArgumentList** to specify the values of parameters in the script.</span></span>

<span data-ttu-id="cab5b-433">當您使用此參數時，PowerShell 會將指定之指令檔的內容轉換成腳本區塊、將腳本區塊傳輸到遠端電腦，然後在遠端電腦上執行它。</span><span class="sxs-lookup"><span data-stu-id="cab5b-433">When you use this parameter, PowerShell converts the contents of the specified script file to a script block, transmits the script block to the remote computer, and runs it on the remote computer.</span></span>
...
### <span data-ttu-id="cab5b-434">-HideComputerName</span><span class="sxs-lookup"><span data-stu-id="cab5b-434">-HideComputerName</span></span>

<span data-ttu-id="cab5b-435">指出此 Cmdlet 會從輸出顯示中省略每個物件的電腦名稱稱。</span><span class="sxs-lookup"><span data-stu-id="cab5b-435">Indicates that this cmdlet omits the computer name of each object from the output display.</span></span> <span data-ttu-id="cab5b-436">預設會在顯示中出現產生物件的電腦名稱。</span><span class="sxs-lookup"><span data-stu-id="cab5b-436">By default, the name of the computer that generated the object appears in the display.</span></span>

<span data-ttu-id="cab5b-437">這個參數只會影響輸出顯示。</span><span class="sxs-lookup"><span data-stu-id="cab5b-437">This parameter affects only the output display.</span></span> <span data-ttu-id="cab5b-438">它不會變更物件。</span><span class="sxs-lookup"><span data-stu-id="cab5b-438">It doesn't change the object.</span></span>
...
### <span data-ttu-id="cab5b-439">-HostName</span><span class="sxs-lookup"><span data-stu-id="cab5b-439">-HostName</span></span>

<span data-ttu-id="cab5b-440">針對安全殼層 (SSH) 基礎的連線，指定電腦名稱稱的陣列。</span><span class="sxs-lookup"><span data-stu-id="cab5b-440">Specifies an array of computer names for a Secure Shell (SSH) based connection.</span></span> <span data-ttu-id="cab5b-441">這類似于 **ComputerName** 參數，不同之處在于遠端電腦的連線是使用 SSH 而非 Windows WinRM 進行。</span><span class="sxs-lookup"><span data-stu-id="cab5b-441">This is similar to the **ComputerName** parameter except that the connection to the remote computer is made using SSH rather than Windows WinRM.</span></span>

<span data-ttu-id="cab5b-442">此參數是在 PowerShell 6.0 中引進。</span><span class="sxs-lookup"><span data-stu-id="cab5b-442">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="cab5b-443">-InDisconnectedSession</span><span class="sxs-lookup"><span data-stu-id="cab5b-443">-InDisconnectedSession</span></span>

<span data-ttu-id="cab5b-444">指出此 Cmdlet 會在已中斷連線的會話中執行命令或腳本。</span><span class="sxs-lookup"><span data-stu-id="cab5b-444">Indicates that this cmdlet runs a command or script in a disconnected session.</span></span>

<span data-ttu-id="cab5b-445">當您使用 **InDisconnectedSession** 參數時，會 `Invoke-Command` 在每一部遠端電腦上建立持續性會話、啟動 **ScriptBlock** 或 **FilePath** 參數所指定的命令，然後與會話中斷連線。</span><span class="sxs-lookup"><span data-stu-id="cab5b-445">When you use the **InDisconnectedSession** parameter, `Invoke-Command` creates a persistent session on each remote computer, starts the command specified by the **ScriptBlock** or **FilePath** parameter, and then disconnects from the session.</span></span> <span data-ttu-id="cab5b-446">命令會繼續在中斷連線的會話中執行。</span><span class="sxs-lookup"><span data-stu-id="cab5b-446">The commands continue to run in the disconnected sessions.</span></span> <span data-ttu-id="cab5b-447">**InDisconnectedSession** 可讓您執行命令，而不需要維護遠端會話的連接。</span><span class="sxs-lookup"><span data-stu-id="cab5b-447">**InDisconnectedSession** enables you to run commands without maintaining a connection to the remote sessions.</span></span> <span data-ttu-id="cab5b-448">而且，由於會話會在傳回任何結果之前中斷連線，因此 **InDisconnectedSession** 會確定所有命令結果都會傳回到重新連線的會話，而不是在會話之間進行分割。</span><span class="sxs-lookup"><span data-stu-id="cab5b-448">And, because the session is disconnected before any results are returned, **InDisconnectedSession** makes sure that all command results are returned to the reconnected session, instead of being split between sessions.</span></span>

<span data-ttu-id="cab5b-449">您無法使用 **InDisconnectedSession** 搭配 **Session** 參數或 **AsJob** 參數。</span><span class="sxs-lookup"><span data-stu-id="cab5b-449">You can't use **InDisconnectedSession** with the **Session** parameter or the **AsJob** parameter.</span></span>

<span data-ttu-id="cab5b-450">使用 **InDisconnectedSession** 的命令會傳回代表中斷連接之會話的 **PSSession** 物件。</span><span class="sxs-lookup"><span data-stu-id="cab5b-450">Commands that use **InDisconnectedSession** return a **PSSession** object that represents the disconnected session.</span></span> <span data-ttu-id="cab5b-451">它們不會傳回命令輸出。</span><span class="sxs-lookup"><span data-stu-id="cab5b-451">They don't return the command output.</span></span> <span data-ttu-id="cab5b-452">若要連接到中斷連線的會話，請使用 `Connect-PSSession` 或 `Receive-PSSession` Cmdlet。</span><span class="sxs-lookup"><span data-stu-id="cab5b-452">To connect to the disconnected session, use the `Connect-PSSession` or `Receive-PSSession` cmdlets.</span></span> <span data-ttu-id="cab5b-453">若要取得會話中執行之命令的結果，請使用 `Receive-PSSession` Cmdlet。</span><span class="sxs-lookup"><span data-stu-id="cab5b-453">To get the results of commands that ran in the session, use the `Receive-PSSession` cmdlet.</span></span> <span data-ttu-id="cab5b-454">若要在中斷連線的會話中執行產生輸出的命令，請將 **OutputBufferingMode** 會話選項的值設為 **Drop** 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-454">To run commands that generate output in a disconnected session, set the value of the **OutputBufferingMode** session option to **Drop** .</span></span> <span data-ttu-id="cab5b-455">如果您想要連線到中斷連線的會話，請在會話中設定閒置超時，讓它提供足夠的時間讓您能夠在刪除會話之前先連接。</span><span class="sxs-lookup"><span data-stu-id="cab5b-455">If you intend to connect to the disconnected session, set the idle time-out in the session so that it provides sufficient time for you to connect before deleting the session.</span></span>

<span data-ttu-id="cab5b-456">您可以在 **SessionOption** 參數或喜好設定變數中設定輸出緩衝處理模式和閒置超時時間 `$PSSessionOption` 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-456">You can set the output buffering mode and idle time-out in the **SessionOption** parameter or in the `$PSSessionOption` preference variable.</span></span> <span data-ttu-id="cab5b-457">如需會話選項的詳細資訊，請參閱 `New-PSSessionOption` 和 [about_Preference_Variables](./about/about_preference_variables.md)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-457">For more information about session options, see `New-PSSessionOption` and [about_Preference_Variables](./about/about_preference_variables.md).</span></span>

<span data-ttu-id="cab5b-458">如需中斷連線工作階段功能的詳細資訊，請參閱 [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-458">For more information about the Disconnected Sessions feature, see [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md).</span></span>

<span data-ttu-id="cab5b-459">此參數是在 PowerShell 3.0 中引進。</span><span class="sxs-lookup"><span data-stu-id="cab5b-459">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="cab5b-460">-InputObject</span><span class="sxs-lookup"><span data-stu-id="cab5b-460">-InputObject</span></span>

<span data-ttu-id="cab5b-461">指定命令的輸入。</span><span class="sxs-lookup"><span data-stu-id="cab5b-461">Specifies input to the command.</span></span> <span data-ttu-id="cab5b-462">輸入包含物件的變數，或輸入可取得物件的命令或運算式。</span><span class="sxs-lookup"><span data-stu-id="cab5b-462">Enter a variable that contains the objects or type a command or expression that gets the objects.</span></span>

<span data-ttu-id="cab5b-463">使用 **InputObject** 參數時，請 `$Input` 在 **ScriptBlock** 參數的值中使用自動變數來代表輸入物件。</span><span class="sxs-lookup"><span data-stu-id="cab5b-463">When using the **InputObject** parameter, use the `$Input` automatic variable in the value of the **ScriptBlock** parameter to represent the input objects.</span></span>
...
### <span data-ttu-id="cab5b-464">-JobName</span><span class="sxs-lookup"><span data-stu-id="cab5b-464">-JobName</span></span>

<span data-ttu-id="cab5b-465">指定背景工作的好記名稱。</span><span class="sxs-lookup"><span data-stu-id="cab5b-465">Specifies a friendly name for the background job.</span></span> <span data-ttu-id="cab5b-466">依預設，作業會命名為 `Job<n>` ，其中 `<n>` 是序號。</span><span class="sxs-lookup"><span data-stu-id="cab5b-466">By default, jobs are named `Job<n>`, where `<n>` is an ordinal number.</span></span>

<span data-ttu-id="cab5b-467">如果您在命令中使用 **JobName** 參數，命令就會以工作的形式執行，並傳回 `Invoke-Command` 工作物件，即使您未在命令中包含 **AsJob** 也一樣。</span><span class="sxs-lookup"><span data-stu-id="cab5b-467">If you use the **JobName** parameter in a command, the command is run as a job, and `Invoke-Command` returns a job object, even if you don't include **AsJob** in the command.</span></span>

<span data-ttu-id="cab5b-468">如需 PowerShell 背景工作的詳細資訊，請參閱 [about_Jobs](./About/about_Jobs.md)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-468">For more information about PowerShell background jobs, see [about_Jobs](./About/about_Jobs.md).</span></span>
...
### <span data-ttu-id="cab5b-469">-KeyFilePath</span><span class="sxs-lookup"><span data-stu-id="cab5b-469">-KeyFilePath</span></span>

<span data-ttu-id="cab5b-470">指定安全殼層 (SSH) 用來驗證遠端電腦上使用者的金鑰檔路徑。</span><span class="sxs-lookup"><span data-stu-id="cab5b-470">Specifies a key file path used by Secure Shell (SSH) to authenticate a user on a remote computer.</span></span>

<span data-ttu-id="cab5b-471">SSH 可讓使用者透過私用金鑰或公開金鑰執行使用者驗證，作為基本密碼驗證的替代方案。</span><span class="sxs-lookup"><span data-stu-id="cab5b-471">SSH allows user authentication to be performed via private and public keys as an alternative to basic password authentication.</span></span> <span data-ttu-id="cab5b-472">如果遠端電腦設定為進行金鑰驗證，則此參數可以用來提供識別使用者的金鑰。</span><span class="sxs-lookup"><span data-stu-id="cab5b-472">If the remote computer is configured for key authentication, then this parameter can be used to provide the key that identifies the user.</span></span>

<span data-ttu-id="cab5b-473">此參數是在 PowerShell 6.0 中引進。</span><span class="sxs-lookup"><span data-stu-id="cab5b-473">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="cab5b-474">-NoNewScope</span><span class="sxs-lookup"><span data-stu-id="cab5b-474">-NoNewScope</span></span>

<span data-ttu-id="cab5b-475">指出此 Cmdlet 會在目前的範圍中執行指定的命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-475">Indicates that this cmdlet runs the specified command in the current scope.</span></span> <span data-ttu-id="cab5b-476">依預設，會 `Invoke-Command` 在自己的範圍中執行命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-476">By default, `Invoke-Command` runs commands in their own scope.</span></span>

<span data-ttu-id="cab5b-477">這個參數只有在於目前的工作階段中執行的命令 (亦即省略 **ComputerName** 和 **Session** 這兩個參數的命令) 中才有效。</span><span class="sxs-lookup"><span data-stu-id="cab5b-477">This parameter is valid only in commands that are run in the current session, that is, commands that omit both the **ComputerName** and **Session** parameters.</span></span>

<span data-ttu-id="cab5b-478">此參數是在 PowerShell 3.0 中引進。</span><span class="sxs-lookup"><span data-stu-id="cab5b-478">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="cab5b-479">-Port</span><span class="sxs-lookup"><span data-stu-id="cab5b-479">-Port</span></span>

<span data-ttu-id="cab5b-480">指定遠端電腦上用於此命令的網路埠。</span><span class="sxs-lookup"><span data-stu-id="cab5b-480">Specifies the network port on the remote computer that is used for this command.</span></span> <span data-ttu-id="cab5b-481">若要連線到遠端電腦，遠端電腦必須接聽連線使用的連接埠。</span><span class="sxs-lookup"><span data-stu-id="cab5b-481">To connect to a remote computer, the remote computer must be listening on the port that the connection uses.</span></span> <span data-ttu-id="cab5b-482">預設通訊埠為 5985 (WinRM 埠（適用于 HTTP）) 和 5986 (適用于 HTTPS) 的 WinRM 埠。</span><span class="sxs-lookup"><span data-stu-id="cab5b-482">The default ports are 5985 (WinRM port for HTTP) and 5986 (WinRM port for HTTPS).</span></span>

<span data-ttu-id="cab5b-483">使用替代連接埠之前，必須先將遠端電腦上的 WinRM 接聽程式設定為在該連接埠進行接聽。</span><span class="sxs-lookup"><span data-stu-id="cab5b-483">Before using an alternate port, configure the WinRM listener on the remote computer to listen at that port.</span></span> <span data-ttu-id="cab5b-484">若要設定接聽程式，請在 PowerShell 提示字元中輸入下列兩個命令：</span><span class="sxs-lookup"><span data-stu-id="cab5b-484">To configure the listener, type the following two commands at the PowerShell prompt:</span></span>

`Remove-Item -Path WSMan:\Localhost\listener\listener* -Recurse`

`New-Item -Path WSMan:\Localhost\listener -Transport http -Address * -Port \<port-number\>`

<span data-ttu-id="cab5b-485">除非您必須這樣做，否則請勿使用 **Port** 參數。</span><span class="sxs-lookup"><span data-stu-id="cab5b-485">Don't use the **Port** parameter unless you must.</span></span> <span data-ttu-id="cab5b-486">在命令中設定的連接埠，將套用於執行命令所在的所有電腦或工作階段。</span><span class="sxs-lookup"><span data-stu-id="cab5b-486">The port that is set in the command applies to all computers or sessions on which the command runs.</span></span> <span data-ttu-id="cab5b-487">替代的連接埠設定可能使得命令無法在全部電腦上執行。</span><span class="sxs-lookup"><span data-stu-id="cab5b-487">An alternate port setting might prevent the command from running on all computers.</span></span>
...
### <span data-ttu-id="cab5b-488">-Remotedebug.js</span><span class="sxs-lookup"><span data-stu-id="cab5b-488">-RemoteDebug</span></span>

<span data-ttu-id="cab5b-489">用來在遠端 PowerShell 會話的偵測模式中執行已叫用的命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-489">Used to run the invoked command in debug mode in the remote PowerShell session.</span></span>
...
### <span data-ttu-id="cab5b-490">-RunAsAdministrator</span><span class="sxs-lookup"><span data-stu-id="cab5b-490">-RunAsAdministrator</span></span>

<span data-ttu-id="cab5b-491">指出此 Cmdlet 會以系統管理員的身分叫用命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-491">Indicates that this cmdlet invokes a command as an Administrator.</span></span>
...
### <span data-ttu-id="cab5b-492">-ScriptBlock</span><span class="sxs-lookup"><span data-stu-id="cab5b-492">-ScriptBlock</span></span>

<span data-ttu-id="cab5b-493">指定要執行的命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-493">Specifies the commands to run.</span></span> <span data-ttu-id="cab5b-494">以大括弧括住命令 `{ }` 以建立腳本區塊。</span><span class="sxs-lookup"><span data-stu-id="cab5b-494">Enclose the commands in curly braces `{ }` to create a script block.</span></span>
<span data-ttu-id="cab5b-495">這是必要參數。</span><span class="sxs-lookup"><span data-stu-id="cab5b-495">This parameter is required.</span></span>

<span data-ttu-id="cab5b-496">預設會在遠端電腦上評估命令中的所有變數。</span><span class="sxs-lookup"><span data-stu-id="cab5b-496">By default, any variables in the command are evaluated on the remote computer.</span></span> <span data-ttu-id="cab5b-497">若要在命令中包含區域變數，請使用 **ArgumentList** 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-497">To include local variables in the command, use **ArgumentList** .</span></span>
...
### <span data-ttu-id="cab5b-498">-Session</span><span class="sxs-lookup"><span data-stu-id="cab5b-498">-Session</span></span>

<span data-ttu-id="cab5b-499">指定此 Cmdlet 在其中執行命令的會話陣列。</span><span class="sxs-lookup"><span data-stu-id="cab5b-499">Specifies an array of sessions in which this cmdlet runs the command.</span></span> <span data-ttu-id="cab5b-500">輸入包含 **pssession** 物件的變數，或輸入可建立或取得 **pssession** 物件的命令，例如 `New-PSSession` 或 `Get-PSSession` 命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-500">Enter a variable that contains **PSSession** objects or a command that creates or gets the **PSSession** objects, such as a `New-PSSession` or `Get-PSSession` command.</span></span>

<span data-ttu-id="cab5b-501">當您建立 **PSSession** 時，PowerShell 會建立與遠端電腦的持續連線。</span><span class="sxs-lookup"><span data-stu-id="cab5b-501">When you create a **PSSession** , PowerShell establishes a persistent connection to the remote computer.</span></span> <span data-ttu-id="cab5b-502">使用 **PSSession** 來執行一系列共用資料的相關命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-502">Use a **PSSession** to run a series of related commands that share data.</span></span> <span data-ttu-id="cab5b-503">若要執行單一命令或一系列不相關的命令，請使用 **ComputerName** 參數。</span><span class="sxs-lookup"><span data-stu-id="cab5b-503">To run a single command or a series of unrelated commands, use the **ComputerName** parameter.</span></span> <span data-ttu-id="cab5b-504">如需詳細資訊，請參閱 [about_PSSessions](./About/about_PSSessions.md)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-504">For more information, see [about_PSSessions](./About/about_PSSessions.md).</span></span>
...
### <span data-ttu-id="cab5b-505">-SessionName</span><span class="sxs-lookup"><span data-stu-id="cab5b-505">-SessionName</span></span>

<span data-ttu-id="cab5b-506">指定中斷連線之工作階段的好記名稱。</span><span class="sxs-lookup"><span data-stu-id="cab5b-506">Specifies a friendly name for a disconnected session.</span></span> <span data-ttu-id="cab5b-507">您可以使用此名稱，在後續命令中參考會話，例如 `Get-PSSession` 命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-507">You can use the name to refer to the session in subsequent commands, such as a `Get-PSSession` command.</span></span> <span data-ttu-id="cab5b-508">這個參數是只有搭配 **InDisconnectedSession** 參數時才有效。</span><span class="sxs-lookup"><span data-stu-id="cab5b-508">This parameter is valid only with the **InDisconnectedSession** parameter.</span></span>

<span data-ttu-id="cab5b-509">此參數是在 PowerShell 3.0 中引進。</span><span class="sxs-lookup"><span data-stu-id="cab5b-509">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="cab5b-510">-SessionOption</span><span class="sxs-lookup"><span data-stu-id="cab5b-510">-SessionOption</span></span>

<span data-ttu-id="cab5b-511">指定會話的 advanced 選項。</span><span class="sxs-lookup"><span data-stu-id="cab5b-511">Specifies advanced options for the session.</span></span> <span data-ttu-id="cab5b-512">輸入 **SessionOption** 物件（例如您使用 Cmdlet 建立的物件 `New-PSSessionOption` ）或雜湊表，其中索引鍵為會話選項名稱，而值為會話選項值。</span><span class="sxs-lookup"><span data-stu-id="cab5b-512">Enter a **SessionOption** object, such as one that you create using the `New-PSSessionOption` cmdlet, or a hash table in which the keys are session option names and the values are session option values.</span></span>

<span data-ttu-id="cab5b-513">選項的預設值取決於 `$PSSessionOption` 喜好設定變數的值（如果已設定）。</span><span class="sxs-lookup"><span data-stu-id="cab5b-513">The default values for the options are determined by the value of the `$PSSessionOption` preference variable, if it's set.</span></span> <span data-ttu-id="cab5b-514">否則，將以工作階段設定中設定的選項建立預設值。</span><span class="sxs-lookup"><span data-stu-id="cab5b-514">Otherwise, the default values are established by options set in the session configuration.</span></span>

<span data-ttu-id="cab5b-515">會話選項值的優先順序高於 `$PSSessionOption` 喜好設定變數和會話設定中設定之會話的預設值。</span><span class="sxs-lookup"><span data-stu-id="cab5b-515">The session option values take precedence over default values for sessions set in the `$PSSessionOption` preference variable and in the session configuration.</span></span> <span data-ttu-id="cab5b-516">不過，它們的優先順序不會高於會話設定中所設定的最大值、配額或限制。</span><span class="sxs-lookup"><span data-stu-id="cab5b-516">However, they don't take precedence over maximum values, quotas, or limits set in the session configuration.</span></span>

<span data-ttu-id="cab5b-517">如需包含預設值之會話選項的描述，請參閱 `New-PSSessionOption` 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-517">For a description of the session options that includes the default values, see `New-PSSessionOption`.</span></span> <span data-ttu-id="cab5b-518">如需喜好設定變數的詳細資訊 `$PSSessionOption` ，請參閱 [about_Preference_Variables](About/about_Preference_Variables.md)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-518">For information about the `$PSSessionOption` preference variable, see [about_Preference_Variables](About/about_Preference_Variables.md).</span></span>
<span data-ttu-id="cab5b-519">如需會話設定的詳細資訊，請參閱 [about_Session_Configurations](About/about_Session_Configurations.md)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-519">For more information about session configurations, see [about_Session_Configurations](About/about_Session_Configurations.md).</span></span>
...
### <span data-ttu-id="cab5b-520">-SSHConnection</span><span class="sxs-lookup"><span data-stu-id="cab5b-520">-SSHConnection</span></span>

<span data-ttu-id="cab5b-521">此參數會採用雜湊表的陣列，其中每個雜湊表包含建立安全殼層 (SSH) 連接所需的一或多個連接參數。</span><span class="sxs-lookup"><span data-stu-id="cab5b-521">This parameter takes an array of hash tables where each hash table contains one or more connection parameters needed to establish a Secure Shell (SSH) connection.</span></span> <span data-ttu-id="cab5b-522">**SSHConnection** 參數適用于建立多個會話，其中每個會話都需要不同的連接資訊。</span><span class="sxs-lookup"><span data-stu-id="cab5b-522">The **SSHConnection** parameter is useful for creating multiple sessions where each session requires different connection information.</span></span>

<span data-ttu-id="cab5b-523">雜湊表具有下列成員：</span><span class="sxs-lookup"><span data-stu-id="cab5b-523">The hashtable has the following members:</span></span>

- <span data-ttu-id="cab5b-524">**ComputerName** (或 **主機名稱** ) </span><span class="sxs-lookup"><span data-stu-id="cab5b-524">**ComputerName** (or **HostName** )</span></span>
- <span data-ttu-id="cab5b-525">**通訊埠**</span><span class="sxs-lookup"><span data-stu-id="cab5b-525">**Port**</span></span>
- <span data-ttu-id="cab5b-526">**使用者名稱**</span><span class="sxs-lookup"><span data-stu-id="cab5b-526">**UserName**</span></span>
- <span data-ttu-id="cab5b-527">**KeyFilePath** (或 **IdentityFilePath** ) </span><span class="sxs-lookup"><span data-stu-id="cab5b-527">**KeyFilePath** (or **IdentityFilePath** )</span></span>

<span data-ttu-id="cab5b-528">**ComputerName** (或 **HostName** ) 是唯一需要的機碼值組。</span><span class="sxs-lookup"><span data-stu-id="cab5b-528">**ComputerName** (or **HostName** ) is the only key-value pair that is required.</span></span>

<span data-ttu-id="cab5b-529">此參數是在 PowerShell 6.0 中引進。</span><span class="sxs-lookup"><span data-stu-id="cab5b-529">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="cab5b-530">-SSHTransport</span><span class="sxs-lookup"><span data-stu-id="cab5b-530">-SSHTransport</span></span>

<span data-ttu-id="cab5b-531">指出使用安全殼層 (SSH) 來建立遠端連線。</span><span class="sxs-lookup"><span data-stu-id="cab5b-531">Indicates that the remote connection is established using Secure Shell (SSH).</span></span>

<span data-ttu-id="cab5b-532">PowerShell 預設會使用 Windows WinRM 連接到遠端電腦。</span><span class="sxs-lookup"><span data-stu-id="cab5b-532">By default PowerShell uses Windows WinRM to connect to a remote computer.</span></span> <span data-ttu-id="cab5b-533">此參數會強制 PowerShell 使用 **HostName** 參數來建立以 SSH 為基礎的遠端連線。</span><span class="sxs-lookup"><span data-stu-id="cab5b-533">This switch forces PowerShell to use the **HostName** parameter for establishing an SSH based remote connection.</span></span>

<span data-ttu-id="cab5b-534">此參數是在 PowerShell 6.0 中引進。</span><span class="sxs-lookup"><span data-stu-id="cab5b-534">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="cab5b-535">-ThrottleLimit</span><span class="sxs-lookup"><span data-stu-id="cab5b-535">-ThrottleLimit</span></span>

<span data-ttu-id="cab5b-536">指定為執行此命令可建立的最大同時連線數。</span><span class="sxs-lookup"><span data-stu-id="cab5b-536">Specifies the maximum number of concurrent connections that can be established to run this command.</span></span>
<span data-ttu-id="cab5b-537">若省略此參數，或輸入值為 0，則會使用預設值 32。</span><span class="sxs-lookup"><span data-stu-id="cab5b-537">If you omit this parameter or enter a value of 0, the default value, 32, is used.</span></span>

<span data-ttu-id="cab5b-538">節流限制僅適用於目前命令，不適用於工作階段或電腦。</span><span class="sxs-lookup"><span data-stu-id="cab5b-538">The throttle limit applies only to the current command, not to the session or to the computer.</span></span>
...
### <span data-ttu-id="cab5b-539">-UserName</span><span class="sxs-lookup"><span data-stu-id="cab5b-539">-UserName</span></span>

<span data-ttu-id="cab5b-540">針對用來在遠端電腦上執行命令的帳戶指定使用者名稱。</span><span class="sxs-lookup"><span data-stu-id="cab5b-540">Specifies the username for the account used to run a command on the remote computer.</span></span> <span data-ttu-id="cab5b-541">使用者驗證方法取決於遠端電腦上如何設定安全殼層 (SSH) 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-541">The user authentication method depends on how Secure Shell (SSH) is configured on the remote computer.</span></span>

<span data-ttu-id="cab5b-542">如果已針對基本密碼驗證設定 SSH，則系統會提示您輸入使用者密碼。</span><span class="sxs-lookup"><span data-stu-id="cab5b-542">If SSH is configured for basic password authentication, then you'll be prompted for the user password.</span></span>

<span data-ttu-id="cab5b-543">如果已針對金鑰型使用者驗證設定 SSH，則可透過 **KeyFilePath** 參數提供金鑰檔案路徑，而且不會出現任何密碼提示。</span><span class="sxs-lookup"><span data-stu-id="cab5b-543">If SSH is configured for key-based user authentication then a key file path can be provided via the **KeyFilePath** parameter and no password prompt occurs.</span></span> <span data-ttu-id="cab5b-544">如果用戶端使用者金鑰檔位於 SSH 已知位置，則不需要 **KeyFilePath** 參數進行金鑰型驗證，而且使用者驗證會根據使用者名稱自動進行。</span><span class="sxs-lookup"><span data-stu-id="cab5b-544">If the client user key file is located in an SSH known location, then the **KeyFilePath** parameter isn't needed for key-based authentication, and user authentication occurs automatically based on the username.</span></span> <span data-ttu-id="cab5b-545">如需詳細資訊，請參閱平臺的 SSH 檔中有關金鑰型使用者驗證的資訊。</span><span class="sxs-lookup"><span data-stu-id="cab5b-545">For more information, see your platform's SSH documentation about key-based user authentication.</span></span>

<span data-ttu-id="cab5b-546">這不是必要的參數。</span><span class="sxs-lookup"><span data-stu-id="cab5b-546">This isn't a required parameter.</span></span> <span data-ttu-id="cab5b-547">如果未指定 **UserName** 參數，則會使用目前登入的使用者名稱來進行連接。</span><span class="sxs-lookup"><span data-stu-id="cab5b-547">If the **UserName** parameter isn't specified, then the current logged on username is used for the connection.</span></span>

<span data-ttu-id="cab5b-548">此參數是在 PowerShell 6.0 中引進。</span><span class="sxs-lookup"><span data-stu-id="cab5b-548">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="cab5b-549">-UseSSL</span><span class="sxs-lookup"><span data-stu-id="cab5b-549">-UseSSL</span></span>

<span data-ttu-id="cab5b-550">指出此 Cmdlet 會使用安全通訊端層 (SSL) 通訊協定來建立與遠端電腦的連線。</span><span class="sxs-lookup"><span data-stu-id="cab5b-550">Indicates that this cmdlet uses the Secure Sockets Layer (SSL) protocol to establish a connection to the remote computer.</span></span> <span data-ttu-id="cab5b-551">預設不會使用 SSL。</span><span class="sxs-lookup"><span data-stu-id="cab5b-551">By default, SSL isn't used.</span></span>

<span data-ttu-id="cab5b-552">WS-Management 加密透過網路傳輸的所有 PowerShell 內容。</span><span class="sxs-lookup"><span data-stu-id="cab5b-552">WS-Management encrypts all PowerShell content transmitted over the network.</span></span> <span data-ttu-id="cab5b-553">**UseSSL** 參數是額外的保護，可跨 HTTPS （而非 HTTP）傳送資料。</span><span class="sxs-lookup"><span data-stu-id="cab5b-553">The **UseSSL** parameter is an additional protection that sends the data across an HTTPS, instead of HTTP.</span></span>

<span data-ttu-id="cab5b-554">如果您使用此參數，但是命令所用的埠上無法使用 SSL，則命令會失敗。</span><span class="sxs-lookup"><span data-stu-id="cab5b-554">If you use this parameter, but SSL isn't available on the port that's used for the command, the command fails.</span></span>
...
### <span data-ttu-id="cab5b-555">-VMId</span><span class="sxs-lookup"><span data-stu-id="cab5b-555">-VMId</span></span>

<span data-ttu-id="cab5b-556">指定虛擬機器的識別碼陣列。</span><span class="sxs-lookup"><span data-stu-id="cab5b-556">Specifies an array of IDs of virtual machines.</span></span>
...
### <span data-ttu-id="cab5b-557">-VMName</span><span class="sxs-lookup"><span data-stu-id="cab5b-557">-VMName</span></span>

<span data-ttu-id="cab5b-558">指定虛擬機器名稱的陣列。</span><span class="sxs-lookup"><span data-stu-id="cab5b-558">Specifies an array of names of virtual machines.</span></span>
...
### <span data-ttu-id="cab5b-559">-子系統</span><span class="sxs-lookup"><span data-stu-id="cab5b-559">-Subsystem</span></span>

<span data-ttu-id="cab5b-560">指定用於新 **PSSession** 的 SSH 子系統。</span><span class="sxs-lookup"><span data-stu-id="cab5b-560">Specifies the SSH subsystem used for the new **PSSession** .</span></span>

<span data-ttu-id="cab5b-561">這會指定要在目標上使用的子系統，如 sshd_config 中所定義。</span><span class="sxs-lookup"><span data-stu-id="cab5b-561">This specifies the subsystem to use on the target as defined in sshd_config.</span></span>
<span data-ttu-id="cab5b-562">子系統會使用預先定義的參數啟動特定版本的 PowerShell。</span><span class="sxs-lookup"><span data-stu-id="cab5b-562">The subsystem starts a specific version of PowerShell with predefined parameters.</span></span>
<span data-ttu-id="cab5b-563">如果指定的子系統不存在於遠端電腦上，則命令會失敗。</span><span class="sxs-lookup"><span data-stu-id="cab5b-563">If the specified subsystem does not exist on the remote computer, the command fails.</span></span>

<span data-ttu-id="cab5b-564">如果未使用此參數，則預設值為 ' powershell ' 子系統。</span><span class="sxs-lookup"><span data-stu-id="cab5b-564">If this parameter is not used, the default is the 'powershell' subsystem.</span></span>
...
### <span data-ttu-id="cab5b-565">CommonParameters</span><span class="sxs-lookup"><span data-stu-id="cab5b-565">CommonParameters</span></span>

<span data-ttu-id="cab5b-566">這個 Cmdlet 支援一般參數：-Debug、-ErrorAction、-ErrorVariable、-InformationAction、-InformationVariable、-OutVariable、-OutBuffer、-PipelineVariable、-Verbose、-WarningAction 和 -WarningVariable。</span><span class="sxs-lookup"><span data-stu-id="cab5b-566">This cmdlet supports the common parameters: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction, and -WarningVariable.</span></span> <span data-ttu-id="cab5b-567">如需詳細資訊，請參閱 [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-567">For more information, see [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216).</span></span>

## <span data-ttu-id="cab5b-568">輸入</span><span class="sxs-lookup"><span data-stu-id="cab5b-568">INPUTS</span></span>

### <span data-ttu-id="cab5b-569">系統管理. ScriptBlock</span><span class="sxs-lookup"><span data-stu-id="cab5b-569">System.Management.Automation.ScriptBlock</span></span>

<span data-ttu-id="cab5b-570">您可以使用管線將腳本區塊中的命令傳送至 `Invoke-Command` 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-570">You can pipe a command in a script block to `Invoke-Command`.</span></span> <span data-ttu-id="cab5b-571">使用 `$Input` 自動變數來代表命令中的輸入物件。</span><span class="sxs-lookup"><span data-stu-id="cab5b-571">Use the `$Input` automatic variable to represent the input objects in the command.</span></span>

## <span data-ttu-id="cab5b-572">輸出</span><span class="sxs-lookup"><span data-stu-id="cab5b-572">OUTPUTS</span></span>

### <span data-ttu-id="cab5b-573">PSRemotingJob、system.servicemodel 或已叫用之命令的輸出（或輸出）。</span><span class="sxs-lookup"><span data-stu-id="cab5b-573">System.Management.Automation.PSRemotingJob, System.Management.Automation.Runspaces.PSSession, or the output of the invoked command</span></span>

<span data-ttu-id="cab5b-574">如果您使用 **AsJob** 參數，此 Cmdlet 會傳回工作物件。</span><span class="sxs-lookup"><span data-stu-id="cab5b-574">This cmdlet returns a job object, if you use the **AsJob** parameter.</span></span> <span data-ttu-id="cab5b-575">如果您指定 **InDisconnectedSession** 參數，則會傳回 `Invoke-Command` **PSSession** 物件。</span><span class="sxs-lookup"><span data-stu-id="cab5b-575">If you specify the **InDisconnectedSession** parameter, `Invoke-Command` returns a **PSSession** object.</span></span> <span data-ttu-id="cab5b-576">否則，它會傳回叫用命令的輸出，這是 **ScriptBlock** 參數的值。</span><span class="sxs-lookup"><span data-stu-id="cab5b-576">Otherwise, it returns the output of the invoked command, which is the value of the **ScriptBlock** parameter.</span></span>

## <span data-ttu-id="cab5b-577">注意</span><span class="sxs-lookup"><span data-stu-id="cab5b-577">NOTES</span></span>

<span data-ttu-id="cab5b-578">在 Windows Vista 和更新版本的 Windows 作業系統上，若要使用的 **ComputerName** 參數在 `Invoke-Command` 本機電腦上執行命令，您必須使用 [以 **系統管理員身分執行** ] 選項來執行 PowerShell。</span><span class="sxs-lookup"><span data-stu-id="cab5b-578">On Windows Vista, and later versions of the Windows operating system, to use the **ComputerName** parameter of `Invoke-Command` to run a command on the local computer, you must run PowerShell using the **Run as administrator** option.</span></span>

<span data-ttu-id="cab5b-579">當您在多部電腦上執行命令時，PowerShell 會依它們出現在清單中的順序連接到電腦。</span><span class="sxs-lookup"><span data-stu-id="cab5b-579">When you run commands on multiple computers, PowerShell connects to the computers in the order in which they appear in the list.</span></span> <span data-ttu-id="cab5b-580">但是，命令輸出會以從遠端電腦收到的順序顯示，可能會不同。</span><span class="sxs-lookup"><span data-stu-id="cab5b-580">However, the command output is displayed in the order that it's received from the remote computers, which might be different.</span></span>

<span data-ttu-id="cab5b-581">執行命令所產生的錯誤 `Invoke-Command` 會包含在命令結果中。</span><span class="sxs-lookup"><span data-stu-id="cab5b-581">Errors that result from the command that `Invoke-Command` runs are included in the command results.</span></span>
<span data-ttu-id="cab5b-582">在本機命令中會是終止錯誤的錯誤在遠端命令中會被視為非終止錯誤。</span><span class="sxs-lookup"><span data-stu-id="cab5b-582">Errors that would be terminating errors in a local command are treated as non-terminating errors in a remote command.</span></span> <span data-ttu-id="cab5b-583">此策略可確保一部電腦上的終止錯誤不會在其執行所在的所有電腦上關閉命令。</span><span class="sxs-lookup"><span data-stu-id="cab5b-583">This strategy makes sure that terminating errors on one computer don't close the command on all computers on which it's run.</span></span> <span data-ttu-id="cab5b-584">甚至是在單一電腦上執行遠端命令時，也會採用這種做法。</span><span class="sxs-lookup"><span data-stu-id="cab5b-584">This practice is used even when a remote command is run on a single computer.</span></span>

<span data-ttu-id="cab5b-585">如果遠端電腦不是在本機電腦信任的網域中，則電腦可能無法驗證使用者的認證。</span><span class="sxs-lookup"><span data-stu-id="cab5b-585">If the remote computer isn't in a domain that the local computer trusts, the computer might not be able to authenticate the user's credentials.</span></span> <span data-ttu-id="cab5b-586">若要將遠端電腦新增至 WS-MANAGEMENT 中受信任的主機清單，請在提供者中使用下列命令 `WSMAN` ，其中 `<Remote-Computer-Name>` 是遠端電腦的名稱：</span><span class="sxs-lookup"><span data-stu-id="cab5b-586">To add the remote computer to the list of trusted hosts in WS-Management, use the following command in the `WSMAN` provider, where `<Remote-Computer-Name>` is the name of the remote computer:</span></span>

`Set-Item -Path WSMan:\Localhost\Client\TrustedHosts -Value \<Remote-Computer-Name\>`

<span data-ttu-id="cab5b-587">當您使用 **InDisconnectedSession** 參數中斷連接 **PSSession** 時，會話狀態會 **中斷** 連線，而且可用性為 **None** 。</span><span class="sxs-lookup"><span data-stu-id="cab5b-587">When you disconnect a **PSSession** using the **InDisconnectedSession** parameter, the session state is **Disconnected** and the availability is **None** .</span></span> <span data-ttu-id="cab5b-588">**State** 屬性的值是相對於目前工作階段。</span><span class="sxs-lookup"><span data-stu-id="cab5b-588">The value of the **State** property is relative to the current session.</span></span> <span data-ttu-id="cab5b-589">值為「已 **中斷** 連線」表示 **PSSession** 未連接到目前的會話。</span><span class="sxs-lookup"><span data-stu-id="cab5b-589">A value of **Disconnected** means that the **PSSession** isn't connected to the current session.</span></span> <span data-ttu-id="cab5b-590">但是，它並不表示 **PSSession** 與所有會話中斷連接。</span><span class="sxs-lookup"><span data-stu-id="cab5b-590">However, it doesn't mean that the **PSSession** is disconnected from all sessions.</span></span> <span data-ttu-id="cab5b-591">它可能連線不同的工作階段。</span><span class="sxs-lookup"><span data-stu-id="cab5b-591">It might be connected to a different session.</span></span> <span data-ttu-id="cab5b-592">若要判斷是否可以連線或重新連線工作階段，請使用 **Availability** 屬性。</span><span class="sxs-lookup"><span data-stu-id="cab5b-592">To determine whether you can connect or reconnect to the session, use the **Availability** property.</span></span>

<span data-ttu-id="cab5b-593">**Availability** 值為 **None** 表示您可以連線工作階段。</span><span class="sxs-lookup"><span data-stu-id="cab5b-593">An **Availability** value of **None** indicates that you can connect to the session.</span></span> <span data-ttu-id="cab5b-594">「 **忙碌** 」值表示您無法連接到 **PSSession** ，因為它已連接到另一個會話。</span><span class="sxs-lookup"><span data-stu-id="cab5b-594">A value of **Busy** indicates that you can't connect to the **PSSession** because it's connected to another session.</span></span> <span data-ttu-id="cab5b-595">如需會話的 **State** 屬性值的詳細資訊，請參閱 [ runspacestate](/dotnet/api/system.management.automation.runspaces.runspacestate)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-595">For more information about the values of the **State** property of sessions, see [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate).</span></span> <span data-ttu-id="cab5b-596">如需會話的 **Availability** 屬性值的詳細資訊，請參閱 [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability)。</span><span class="sxs-lookup"><span data-stu-id="cab5b-596">For more information about the values of the **Availability** property of sessions, see [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability).</span></span>

<span data-ttu-id="cab5b-597">**主機名稱** 和 **SSHConnection** 參數已包含在 PowerShell 6.0 的開頭。</span><span class="sxs-lookup"><span data-stu-id="cab5b-597">The **HostName** and **SSHConnection** parameters were included starting with PowerShell 6.0.</span></span> <span data-ttu-id="cab5b-598">已新增它們，以根據安全殼層 (SSH) 提供 PowerShell 遠端功能。</span><span class="sxs-lookup"><span data-stu-id="cab5b-598">They were added to provide PowerShell remoting based on Secure Shell (SSH).</span></span> <span data-ttu-id="cab5b-599">在多個平臺上支援 PowerShell 和 SSH (Windows、Linux、macOS) 和 PowerShell 遠端處理可在安裝和設定 PowerShell 和 SSH 的平臺上運作。</span><span class="sxs-lookup"><span data-stu-id="cab5b-599">PowerShell and SSH are supported on multiple platforms (Windows, Linux, macOS) and PowerShell remoting works over these platforms where PowerShell and SSH are installed and configured.</span></span> <span data-ttu-id="cab5b-600">這與先前僅限 Windows 的遠端處理（以 WinRM 為基礎）和許多 WinRM 特定功能和限制都不適用。</span><span class="sxs-lookup"><span data-stu-id="cab5b-600">This is separate from the previous Windows only remoting that is based on WinRM and many of the WinRM specific features and limitations don't apply.</span></span> <span data-ttu-id="cab5b-601">例如，目前不支援以 WinRM 為基礎的配額、會話選項、自訂端點設定以及中斷連線/重新連線功能。</span><span class="sxs-lookup"><span data-stu-id="cab5b-601">For example WinRM based quotas, session options, custom endpoint configuration, and disconnect/reconnect features are currently not supported.</span></span> <span data-ttu-id="cab5b-602">如需有關如何設定 PowerShell SSH 遠端的詳細資訊，請參閱透過 [SSH 的 Powershell 遠端](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core)功能。</span><span class="sxs-lookup"><span data-stu-id="cab5b-602">For more information about how to set up PowerShell SSH remoting, see [PowerShell Remoting Over SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span></span>

## <span data-ttu-id="cab5b-603">相關連結</span><span class="sxs-lookup"><span data-stu-id="cab5b-603">RELATED LINKS</span></span>

[<span data-ttu-id="cab5b-604">about_PSSessions</span><span class="sxs-lookup"><span data-stu-id="cab5b-604">about_PSSessions</span></span>](./About/about_PSSessions.md)

[<span data-ttu-id="cab5b-605">about_Remote</span><span class="sxs-lookup"><span data-stu-id="cab5b-605">about_Remote</span></span>](./About/about_Remote.md)

[<span data-ttu-id="cab5b-606">about_Remote_Disconnected_Sessions</span><span class="sxs-lookup"><span data-stu-id="cab5b-606">about_Remote_Disconnected_Sessions</span></span>](./About/about_Remote_Disconnected_Sessions.md)

[<span data-ttu-id="cab5b-607">about_Remote_Troubleshooting</span><span class="sxs-lookup"><span data-stu-id="cab5b-607">about_Remote_Troubleshooting</span></span>](./About/about_remote_troubleshooting.md)

[<span data-ttu-id="cab5b-608">about_Remote_Variables</span><span class="sxs-lookup"><span data-stu-id="cab5b-608">about_Remote_Variables</span></span>](./About/about_Remote_Variables.md)

[<span data-ttu-id="cab5b-609">about_Scopes</span><span class="sxs-lookup"><span data-stu-id="cab5b-609">about_Scopes</span></span>](./About/about_scopes.md)

[<span data-ttu-id="cab5b-610">Enter-PSSession</span><span class="sxs-lookup"><span data-stu-id="cab5b-610">Enter-PSSession</span></span>](Enter-PSSession.md)

[<span data-ttu-id="cab5b-611">Exit-PSSession</span><span class="sxs-lookup"><span data-stu-id="cab5b-611">Exit-PSSession</span></span>](Exit-PSSession.md)

[<span data-ttu-id="cab5b-612">Get-PSSession</span><span class="sxs-lookup"><span data-stu-id="cab5b-612">Get-PSSession</span></span>](Get-PSSession.md)

[<span data-ttu-id="cab5b-613">Invoke-Item</span><span class="sxs-lookup"><span data-stu-id="cab5b-613">Invoke-Item</span></span>](../Microsoft.PowerShell.Management/Invoke-Item.md)

[<span data-ttu-id="cab5b-614">New-PSSession</span><span class="sxs-lookup"><span data-stu-id="cab5b-614">New-PSSession</span></span>](New-PSSession.md)

[<span data-ttu-id="cab5b-615">Remove-PSSession</span><span class="sxs-lookup"><span data-stu-id="cab5b-615">Remove-PSSession</span></span>](Remove-PSSession.md)

[<span data-ttu-id="cab5b-616">WSMan 提供者</span><span class="sxs-lookup"><span data-stu-id="cab5b-616">WSMan Provider</span></span>](../Microsoft.WsMan.Management/About/about_WSMan_Provider.md)
