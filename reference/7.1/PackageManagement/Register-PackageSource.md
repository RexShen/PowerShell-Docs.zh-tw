...
指令 `Register-PackageSource` 程式會加入指定封裝提供者的封裝來源。 套件來源一律受套件提供者管理。 如果套件提供者不能新增或取代套件來源，提供者會產生錯誤訊息。

## 範例

### 範例 1：註冊 NuGet 提供者的套件來源

此命令會註冊一個套件來源（ **NuGet** 提供者的 web 型位置）。 依預設，來源不受信任。 安裝封裝之前，系統會提示您確認來源是受信任的。 若要覆寫預設值，請使用 `-Trusted` 參數。
