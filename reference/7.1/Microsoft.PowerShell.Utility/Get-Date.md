...
### 範例10：傳回以 UTC 格式轉譯的日期值

此範例示範如何將日期值解讀為其對等的 UTC。 在此範例中，此機器設定為 **太平洋標準時間**。 根據預設， `Get-Date` 會傳回該時區的值。 使用 **AsUTC** 參數將值轉換為 UTC 相等時間。
...
自1970年1月1日起，0:00:00 起的日期和時間，以秒為單位。
...
| `%F` | 以 YYYY-MM-DD 格式的日期（等於% Y-% m-% d） (ISO 8601 日期格式)  | 2019-06-27               |
| `%G` | 與 ' Y ' 相同                                                             |                          |
| `%g` | 與 ' y ' 相同                                                             |                          |
| `%H` | 24小時制的小時                                                  | 17                       |
| `%h` | 與 ' b ' 相同                                                             |                          |
...
| `%S` | 秒                                                                 | 05                       |
| `%s` | 自 00:00:00 1970 年1月1日起經過的秒數                          | 1150451174               |
| `%t` | 水準定位字元                                                |                          |
