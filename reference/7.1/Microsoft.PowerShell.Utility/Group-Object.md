...
從 PowerShell 7 開始， `Group-Object` 可以結合 **CaseSensitive** 和 **AsHashtable** 參數來建立區分大小寫的雜湊表。 雜湊表索引鍵會使用區分大小寫的比較，並輸出 **system.string 物件。**
...
此範例結合了 **CaseSensitive** 和 **AsHashTable** 參數來建立區分大小寫的雜湊表。 範例中的檔案具有和的副檔名 `.txt` `.TXT` 。
...
`$hash`變數會儲存 **system.object** 物件。 `Get-ChildItem` 從目錄中取得檔案名 `C:\Files` ，然後將 **FileInfo** 物件傳送至管線。 `Group-Object` 使用 **屬性** 值 **延伸** 來將物件分組。 **CaseSensitive** 和 **AsHashTable** 參數會建立雜湊表，而且索引鍵會使用區分大小寫的索引鍵和來分組 `.txt` `.TXT` 。
...
從 PowerShell 7 開始，若要建立區分大小寫的雜湊表，請在命令中包含 **CaseSensitive** 和 **AsHashtable** 。
