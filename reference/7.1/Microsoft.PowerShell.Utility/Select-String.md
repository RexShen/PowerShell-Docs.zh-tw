...
指定要與指定模式相符的文化特性名稱。 **Culture** 參數必須搭配 **SimpleMatch** 參數使用。 預設行為會使用目前 PowerShell 運行時 (會話) 的文化特性。

若要取得所有支援的文化特性清單，請使用 `Get-Culture -ListAvailable` 命令。
...
使用 `Select-String -Culture Ordinal -CaseSensitive -SimpleMatch` 命令可讓您取得最快速的二進位比較。

**Culture** 參數會使用 tab 鍵自動完成，以在指定可用文化特性的引數清單中進行滾動。 若要列出所有可用的引數，請使用下列命令：

`(Get-Command Select-String).Parameters.Culture.Attributes.ValidValues`

如需 .NET CultureInfo.Name 屬性的詳細資訊，請參閱 [CultureInfo.Name](/dotnet/api//system.globalization.cultureinfo.name)。

**Culture** 參數是在 PowerShell 7 中引進。
...
依預設，會反白 `Select-String` 顯示符合您使用 **pattern** 參數搜尋之模式的字串。 **NoEmphasis** 參數會停用反白顯示。

強調會根據您的 PowerShell 背景和文字色彩來使用負面色彩。 例如，如果您的 PowerShell 色彩是具有白色文字的黑色背景。 強調是黑色文字的白色背景。
...
使 Cmdlet 只輸出相符的字串，而不是 **MatchInfo** 物件。 這是最類似 Unix **grep** 或 Windows **findstr.exe** 命令的行為結果。
...
### MatchInfo，System.string，System.string.. 字串

根據預設，輸出是一組 **MatchInfo** 物件，每個找到的相符項都有一個物件。 如果您使用 **Quiet** 參數，則輸出會是 **布林** 值，指出是否已找到模式。
如果您使用 **Raw** 參數，則輸出會是一組符合模式的 **字串** 物件。
