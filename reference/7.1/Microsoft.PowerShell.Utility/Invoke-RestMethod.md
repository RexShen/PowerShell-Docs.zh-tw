...
將回應主體儲存在指定的輸出檔中。 輸入路徑和檔案名稱。 若省略路徑，則預設為目前位置。 名稱會被視為常值路徑。 包含括弧 () 的名稱 `[]` 必須用單引號括住 (`'`) 。
...
> [!NOTE]
> 在非 Windows 平臺上，可能無法提供 `Tls` 或 `Tls12` 作為選項。 `Tls13`並非所有作業系統都可使用的支援，而且必須根據每個作業系統進行驗證。

這項功能已新增至 powershell 6.0.0，且 `Tls13` 已在 powershell 7.1 中新增的支援。
...
- 若 **為 Windows** ：從使用者的 proxy 設定讀取環境變數中的 proxy 設定，或從使用者的 proxy 設定讀取 proxy 設定（如果未定義的話）。
- **針對 macOS** ：從環境變數讀取 proxy 設定，如果未定義，則從系統的 proxy 設定。
- 若 **為 Linux** ：從環境變數讀取 proxy 設定，如果未定義，則這個屬性會初始化略過所有位址的未設定實例。
