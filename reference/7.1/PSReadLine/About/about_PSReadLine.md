...
刪除多行緩衝區中之前要求的邏輯行和目前的邏輯行。

- Vi 命令模式： `<d,k>`

### <a name="deleterelativelines"></a>DeleteRelativeLines

從緩衝區的開頭刪除多行緩衝區中目前的邏輯行。

最多有 Vi 個命令， `<d,g,g>` 命令前面會加上指定絕對行號的數值引數，這會和目前的行號一起組成要刪除的行範圍。
如果未指定，則數值引數會預設為1，這會參考多行緩衝區中的第一個邏輯行。

要從多行刪除的實際行數，會計算為目前的邏輯行號和指定的數值引數之間的差異，因此可能是負數。 因此，方法名稱的 _相對_ 部分。

- Vi 命令模式： `<d,g,g>`

### <a name="deletenextlines"></a>DeleteNextLines

刪除多行緩衝區中目前的邏輯行和下一個要求的邏輯行。
...
將游標移至 Vi 編輯模式的左邊一個字元。 這可能會將游標移至多行輸入的上一行。
...
將游標移至 Vi 編輯模式的右側一個字元。 這可能會將游標移至下一行的多行輸入。
...
這兩個函數是由所使用 `Get-PSReadLineKeyHandler` 。 第一個用來取得所有按鍵系結。 第二個是用來取得特定的索引鍵系結。
